...
if (! $action || $action == 'profile') {
    ...
    $typeid = empty($typeid) || ! in_array($typeid, array(
        1,
        2,
        3,
        4,
    ...
    if (! submitcheck('editsubmit', 0, $seccodecheck)) {
        ...
        if ($typeid == 1) {
            ...
            $gendercheck = array(
                $member['gender'] => 'selected="selected"'
            );
            ...
            $tppchecked = array(
                $member['tpp'] => 'checked="checked"'
            );
            $pppchecked = array(
                $member['ppp'] => 'checked="checked"'
            );
            $toselect = array(
                strval((float) $member['timeoffset']) => 'selected="selected"'
            );
            $pscheck = array(
                intval($member['pmsound']) => 'checked="checked"'
            );
            $emcheck = array(
                $member['editormode'] => 'checked="checked"'
            );
            $tfcheck = array(
                $member['timeformat'] => 'checked="checked"'
            );
            $dfcheck = array(
                $member['dateformat'] => 'checked="checked"'
            );
            ...
            $dateconvertchecked = array(
                $customshow{0} => 'checked="checked"'
            );
            $sschecked = array(
                $customshow{1} => 'checked="checked"'
            );
            $sachecked = array(
                $customshow{2} => 'checked="checked"'
            );
            $sichecked = array(
                $customshow{3} => 'checked="checked"'
            );
            
            $creditnoticechecked = array(
                empty($_COOKIE['discuz_creditnoticedisable']) => 'checked="checked"'
            );
            
            $dateformatlist = array();
            if (! empty($userdateformat) && ($count = count($userdateformat))) {
                for ($num = 1; $num <= $count; $num ++) {
                    $dateformatlist[$num] = str_replace(array(
                        'n',
                        'j',
                        'y',
                        'Y'
        ...
        if ($typeid == 1) {
            ...
            if ($msnnew && ! iswechat($msnnew)) {
                showmessage('profile_wechat_illegal');
            }
            ...
            $tppnew = in_array($tppnew, array(
                10,
                20,
                30
            )) ? $tppnew : 0;
            $pppnew = in_array($pppnew, array(
                5,
                10,
                15
            )) ? $pppnew : 0;
            $editormodenew = in_array($editormodenew, array(
                0,
                1,
                2
            )) ? $editormodenew : 2;
            $ssnew = in_array($ssnew, array(
                0,
                1
            )) ? $ssnew : 2;
            $sanew = in_array($sanew, array(
                0,
                1
            )) ? $sanew : 2;
            $sinew = in_array($sinew, array(
                0,
                1
            )) ? $sinew : 2;
        ...
        if (! empty($extcredits)) {
            foreach ($extcredits as $key => $value) {
                if ($value['allowexchangein'] || $value['allowexchangeout']) {
                    $extcredits_exchange['extcredits' . $key] = array(
                        'title' => $value['title'],
                        'unit' => $value['unit']
                    );
                }
    ...
    if (empty($type)) {
        ...
        if ($type == 'main' && $switchmaingroup) {
            ...
            $adminidnew = in_array($adminid, array(
                1,
                2,
                3
            )) ? $adminid : ($group['type'] == 'special' ? - 1 : 0);
            ...
            if (! submitcheck('groupsubmit')) {
                ...
                if ($join) {
                    ...
                    foreach (array_unique(array_merge(explode("\t", $extgroupids), array(
                        $edit
                    ))) as $extgroupid) {
                        if ($extgroupid) {
