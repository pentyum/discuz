...
$navtitle = $thread['subject'] . '(' . $lang['page'] . ' ' . $page . ') ' . ($thread['type'] == 'sub' ? ' - ' . strip_tags($_DCACHE['forums'][$thread['fup']]['name']) : '') . ' - ' . strip_tags($thread['name']) . ' - ';
...
if (! $thread || ! (! $thread['viewperm'] || ($thread['viewperm'] && forumperm($thread['viewperm']))) || ! forumformulaperm($thread['formulaperm'])) {
    ...
    if ($firstpost = $sdb->fetch_array($query)) {
        if (in_array($firstpost['groupid'], array(
            4,
            5,
            6
        ))) {
        ...
        $meta_contentadd = cutstr(strip_tags(str_replace(array(
            "\r",
            "\n",
            "\t"
        ), array(
    ...
    while (($post = $firstpost) || ($post = $sdb->fetch_array($query))) {
        if (in_array($post['groupid'], array(
            4,
            5,
            6
        ))) {
        ...
        $post['message'] = ($post['subject'] ? '<h3>' . $post['subject'] . '</h3>' : '') . nl2br(preg_replace(array(
            '/&amp;(#\d{3,5};)/',
            "/\[hide=?\d*\](.+?)\[\/hide\]/is"
        ), array(
            '&\\1',
...
function jammer()
{
    ...
    return mt_rand(0, 1) ? '<font style="font-size:0px;color:' . ALTBG2 . '">' . $randomstr . '</font><br />' : '<br /><span style="display:none">' . $randomstr . '</span>';
}
