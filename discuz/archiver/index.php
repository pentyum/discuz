...
$fullversion = array(
    'title' => $_DCACHE['settings']['bbname'],
    'link' => $_DCACHE['settings']['indexname']
);
...
if ($querystring) {
    ...
    foreach ($queryparts as $querypart) {
        if (empty($lastpart)) {
            $lastpart = in_array($querypart, array(
                'fid',
                'page',
                'tid'
            )) ? $querypart : '';
...
if ($_DCACHE['settings']['archiverstatus'] != 1 && ! preg_match("/($kw_spiders)/i", $_SERVER['HTTP_USER_AGENT']) && (($_DCACHE['settings']['archiverstatus'] == 2 && preg_match("/($kw_searchengines)/", $_SERVER['HTTP_REFERER'])) || ($_DCACHE['settings']['archiverstatus'] == 3 && preg_match("/($kw_browsers)/", $_SERVER['HTTP_USER_AGENT'])))) {
