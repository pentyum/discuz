...
if ((empty($type) && empty($statstatus)) || (isset($type) && $type == 'posts')) {
    ...
    while ($variable = $db->fetch_array($query)) {
        $variable['type'] == 'monthposts' ? $stats_monthposts[$variable['variable']] = $variable['value'] : $stats_dayposts[$variable['variable']] = $variable['value'];
    }
...
if (empty($type)) {
...
} elseif ($type == 'posts' || ($statstatus && in_array($type, array(
    'views',
    'agent'
)))) {
    ...
    switch ($type) {
        case 'views':
            ...
            break;
        case 'agent':
            $statsbar_browser = statsdata('browser', $maxbrowser, 'arsort');
