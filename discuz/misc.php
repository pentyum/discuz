...
if ($action == 'maxpages') {
    ...
    if (! submitcheck('paysubmit')) {
        ...
        if ($updateauthor) {
            updatecredits($attach['uid'], array(
                $creditstransextra[1] => $attach['netprice']
            ));
        }
        updatecredits($discuz_uid, array(
            $creditstransextra[1] => $attach['price']
        ), - 1);
...
if ($action == 'votepoll' && submitcheck('pollsubmit', 1)) {
    ...
    while ($options = $db->fetch_array($query)) {
        ...
        $options['polloption'] = preg_replace("/\[url=(https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|ed2k|thunder|synacast){1}:\/\/([^\[\"']+?)\](.+?)\[\/url\]/i", "<a href=\"\\1://\\2\" target=\"_blank\">\\3</a>", $options['polloption']);
        $polloptions[] = $options;
    }
    ...
    if (! submitcheck('ratesubmit')) {
        ...
        if ($post['first']) {
            ...
            $send_feed = false;
            if (is_array($dzfeed_limit['thread_rate']))
                foreach ($dzfeed_limit['thread_rate'] as $val) {
            ...
            $send_feed = false;
            if (is_array($dzfeed_limit['post_rate']))
                foreach ($dzfeed_limit['post_rate'] as $val) {
                    if ($post['rate'] < $val && ($post['rate'] + $rate) > $val) {
                        $send_feed = true;
                        $count = $val;
                    }
        ...
        if (! empty($logidarray)) {
            ...
            while ($ratelog = $db->fetch_array($query)) {
                if (in_array($ratelog['uid'] . ' ' . $ratelog['extcredits'] . ' ' . $ratelog['dateline'], $logidarray)) {
                    ...
                    $ratetimes += ceil(max(abs($rating['min']), abs($rating['max'])) / 5);
                    updatecredits($post['authorid'], array(
    ...
    if (! submitcheck('paysubmit')) {
        ...
        if ($updateauthor) {
            updatecredits($thread['authorid'], array(
                $creditstransextra[1] => $thread['netprice']
            ));
        }
        updatecredits($discuz_uid, array(
            $creditstransextra[1] => $thread['price']
        ), - 1);
