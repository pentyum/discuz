...
if ($action == 'clearcookies') {
    ...
    $type = isset($type) && in_array($type, array(
        'admins',
        'birthdays',
        'grouplist'
    )) ? $type : '';
    $order = isset($order) && in_array($order, array(
        'credits',
        'gender',
        'username'
    )) ? $order : '';
    ...
    switch ($type) {
        ...
        default:
            ...
            $sql = ! empty($srchmem) ? " WHERE username LIKE '" . str_replace(array(
                '_',
                '%'
            ), array(
                '\_',
    ...
    if (! empty($groupterms['ext'])) {
        foreach ($groupterms['ext'] as $extgroupid => $time) {
            $expirylist[$extgroupid] = array(
                'time' => gmdate($dateformat, $time + $timeoffset * 3600),
                'type' => 'ext'
            );
        }
    }
    ...
    if (! empty($groupterms['main'])) {
        $expirylist[$groupid] = array(
            'time' => gmdate($dateformat, $groupterms['main']['time'] + $timeoffset * 3600),
            'type' => 'main'
        );
    }
