...
if ($apptype == 'discuz') {
    ...
    $tablepre = $tablepre;
    if (empty($dbcharset)) {
        $dbcharset = in_array(strtolower($charset), array(
            'gbk',
            'big5',
            'utf-8'
        )) ? str_replace('-', '', $charset) : '';
    ...
    while ($currsize + strlen($tabledump) + 500 < $sizelimit * 1000 && $numrows == $offset) {
        ...
        while ($row = $db->fetch_row($rows)) {
            ...
            for ($i = 0; $i < $numfields; $i ++) {
                $t .= $comma . ($usehex && ! empty($row[$i]) && (strexists($tablefields[$i]['Type'], 'char') || strexists($tablefields[$i]['Type'], 'text')) ? '0x' . bin2hex($row[$i]) : '\'' . mysqli_escape_string($row[$i]) . '\'');
                $comma = ',';
            }
