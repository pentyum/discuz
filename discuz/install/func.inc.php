...
function createtable($sql)
{
    ...
    return preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $sql) . (mysqli_get_server_info($db->link) > '4.1' ? " ENGINE=$type DEFAULT CHARSET=" . DBCHARSET : " TYPE=$type");
}
...
{
    echo "<script>" . "function redirect() {window.location.replace('$url');}\n" . "setTimeout('redirect();', 0);\n" . "</script>";
    exit();
}
...
<div class="main">
	...
	<div class="btnbox marginbot">
		<input type="button" name="submit"
			value="<?=lang('install_in_processed')?>" disabled style="height: 25"
			id="laststep" onclick="initinput()">
	</div>
<?php
}
