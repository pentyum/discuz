...
if (! empty($tid)) {
    ...
    if (($thread['special'] && in_array($action, array(
        'copy',
        'split',
        'merge'
    ))) || ($thread['digest'] == '-1' && ! in_array($action, array(
        'delpost',
        'banpost',
        'getip'
    )))) {
...
if ($action == 'moderate') {
    ...
    if (! submitcheck('modsubmit')) {
        ...
        $resultarray = array(
            ...
            'reasonpm' => ($sendreasonpm ? array(
                'data' => $posts,
                'var' => 'post',
                'item' => 'reason_delete_post'
            ) : array()),
            'modtids' => 0,
            'modlog' => $thread
        );
    ...
    if (! submitcheck('modsubmit')) {
        ...
        $resultarray = array(
            ...
            'reasonpm' => ($sendreasonpm ? array(
                ...
                'var' => 'thread',
                'item' => $stamp !== '' ? 'reason_stamp_update' : 'reason_stamp_delete'
            ) : array()),
