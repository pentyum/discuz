...
if (! empty($orderid)) {
    ...
    if (! empty($pay) && ! $tradelog['offline'] && $tradelog['status'] == 0 && $tradelog['buyerid'] == $discuz_uid) {
        if ($creditstransextra[5] != - 1 && $tradelog['credit']) {
            ...
            updatecredits($tradelog['buyerid'], array(
                $creditstransextra[5] => - $tradelog['credit']
            ));
        }
        ...
        $transportfee = 0;
        trade_setprice(array(
            'fee' => $fee,
            'trade' => $trade,
            'transport' => $transport
        ), $price, $pay, $transportfee);
    ...
    if (submitcheck('offlinesubmit') && in_array($offlinestatus, trade_offline($tradelog, 0))) {
        ...
        if ($offlinestatus == STATUS_SELLER_SEND) {
            ...
            if ($creditstransextra[5] != - 1 && $tradelog['basecredit']) {
                ...
                updatecredits($tradelog['sellerid'], array(
                    $creditstransextra[5] => $netcredit
                ));
            } else {
                $netcredit = 0;
            }
            ...
            if ($creditstransextra[5] != - 1 && $tradelog['basecredit']) {
                updatecredits($tradelog['buyerid'], array(
                    $creditstransextra[5] => $tradelog['number'] * $tradelog['basecredit']
                ));
            }
        }
