...
if ($action == 'logout' && ! empty($formhash)) {
    ...
    if (! submitcheck('loginsubmit', 1, $seccodecheck)) {
        ...
        $thetimenow = '(GMT ' . ($timeoffset > 0 ? '+' : '') . $timeoffset . ') ' . dgmdate("$dateformat $timeformat", $timestamp + $timeoffset * 3600) . 
        $styleselect = '';
        ...
        if ($result > 0) {
            ...
            $errorlog = dhtmlspecialchars($timestamp . "\t" . ($ucresult['username'] ? $ucresult['username'] : stripslashes($username)) . "\t" . $password . "\t" . ($secques ? "Ques #" . intval($questionid) : '') . "\t" . $onlineip);
