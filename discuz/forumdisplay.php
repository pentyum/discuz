...
if ($forum['modworks']) {
    ...
    $modnum = $allowmodpost ? ($db->result_first("SELECT COUNT(*) FROM {$tablepre}posts WHERE invisible='-2' AND first='0' and fid='$fid'") + $db->result_first("SELECT COUNT(*) FROM {$tablepre}threads WHERE fid='$fid' AND displayorder='-2'")) : 0;
...
foreach ($_DCACHE['forums'] as $sub) {
    if ($sub['type'] == 'sub' && $sub['fup'] == $fid && (! $hideprivate || ! $sub['viewperm'] || forumperm($sub['viewperm']) || strstr($sub['users'], "\t$discuz_uid\t"))) {
        ...
        $sql = $accessmasks ? "SELECT f.fid, f.fup, f.type, f.name, f.threads, f.posts, f.todayposts, f.lastpost, ff.description, ff.moderators, ff.icon, ff.viewperm, ff.extra, a.allowview FROM {$tablepre}forums f
						...
						WHERE fup='$fid' AND status>'0' AND type='sub' ORDER BY f.displayorder" : "SELECT f.fid, f.fup, f.type, f.name, f.threads, f.posts, f.todayposts, f.lastpost, ff.description, ff.moderators, ff.icon, ff.viewperm, ff.extra FROM {$tablepre}forums f
...
isset($orderby) && in_array($orderby, array(
    'lastpost',
    'dateline',
    'replies',
    'views',
    'recommends',
    'heats'
)) ? $forumdisplayadd .= "&amp;orderby=$orderby" : $orderby = $_DCACHE['forums'][$fid]['orderby'] ? $_DCACHE['forums'][$fid]['orderby'] : 'lastpost';
isset($ascdesc) && in_array($ascdesc, array(
    'ASC',
    'DESC'
)) ? $forumdisplayadd .= "&amp;ascdesc=$ascdesc" : $ascdesc = $_DCACHE['forums'][$fid]['ascdesc'] ? $_DCACHE['forums'][$fid]['ascdesc'] : 'DESC';
            ...
            if ($selectadd && is_array($selectadd)) {
                ...
                foreach ($quicksearchlist['option'] as $option) {
                    ...
                    foreach ($selectadd as $option => $value) {
                        $sorturladd[$identifier] .= ! in_array($option, array(
                            'filter',
                            'sortid',
                            $identifier
                        )) ? "$and$option=$value" : '';
...
$filterbool = ! empty($filter) && in_array($filter, array(
    'digest',
    'recommend',
    'type',
    'activity',
...
while (($querysticky && $thread = $sdb->fetch_array($querysticky)) || ($query && $thread = $sdb->fetch_array($query))) {
    ...
    $thread['typeid'] = $thread['typeid'] && ! empty($forum['threadtypes']['prefix']) && isset($forum['threadtypes']['types'][$thread['typeid']]) ? '<em>[<a href="forumdisplay.php?fid=' . $fid . '&amp;filter=type&amp;typeid=' . $thread['typeid'] . '">' . $forum['threadtypes']['types'][$thread['typeid']] . '</a>]</em>' : '';
    
    $thread['sortid'] = $thread['sortid'] && ! empty($forum['threadsorts']['prefix']) && isset($forum['threadsorts']['types'][$thread['sortid']]) ? '<em>[<a href="forumdisplay.php?fid=' . $fid . '&amp;filter=sort&amp;sortid=' . $thread['sortid'] . '">' . $forum['threadsorts']['types'][$thread['sortid']] . '</a>]</em>' : '';
    $thread['multipage'] = '';
    ...
    if (in_array($thread['displayorder'], array(
        1,
        2,
        3,
        4
