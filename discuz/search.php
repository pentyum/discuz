...
if (! submitcheck('searchsubmit', 1)) {
    ...
    if (! empty($searchid)) {
        ...
        $srchfilter = in_array($srchfilter, array(
            'all',
            'digest',
            'top'
        )) ? $srchfilter : 'all';
        ...
        if ($searchindex['id']) {
            ...
            if (! $srchtxt && ! $srchuid && ! $srchuname && ! $srchfrom && ! in_array($srchfilter, array(
                'digest',
                'top'
            )) && ! is_array($special)) {
            ...
            if (! empty($srchfrom) && empty($srchtxt) && empty($srchuid) && empty($srchuname)) {
                ...
                $sqlsrch = $srchtype == 'fulltext' ? "FROM {$tablepre}posts p, {$tablepre}threads t WHERE $digestltd t.fid IN ($fids) $topltd AND p.tid=t.tid AND p.invisible='0'" : "FROM {$tablepre}threads t WHERE $digestltd t.fid IN ($fids) $topltd";
