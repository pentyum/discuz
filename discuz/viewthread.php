    ...
    if ($forum['threadsorts']['types'][$thread['sortid']]) {
        if (@include_once DISCUZ_ROOT . './forumdata/cache/threadsort_' . $thread['sortid'] . '.php') {
            ...
            foreach ($_DTYPE as $optionid => $option) {
                ...
                } elseif (in_array($_DTYPE[$optionid]['type'], array(
                    'radio',
                    'select'
                ))) {
...
$thread['subject'] = ($forum['threadsorts']['types'][$thread['sortid']] ? ($forum['threadsorts']['listable'] ? '<a href="forumdisplay.php?fid=' . $fid . '&amp;filter=sort&amp;sortid=' . $thread['sortid'] . '">[' . $forum['threadsorts']['types'][$thread['sortid']] . ']</a>' : '[' . $forum['threadsorts']['types'][$thread['sortid']] . ']') . ' ' : '') . ($forum['threadtypes']['types'][$thread['typeid']] ? ($forum['threadtypes']['listable'] ? '<a href="forumdisplay.php?fid=' . $fid . '&amp;filter=type&amp;typeid=' . $thread['typeid'] . '">[' . $forum['threadtypes']['types'][$thread['typeid']] . ']</a>' : '[' . $forum['threadtypes']['types'][$thread['typeid']] . ']') . ' ' : '') . $thread['subject'];
...
if (empty($action) && $tid) {
    ...
    if ($thread['special'] > 0 && (empty($viewpid) || $viewpid == $firstpid)) {
        ...
        switch ($thread['special']) {
            case 1:
                ...
                break;
            case 2:
                include_once DISCUZ_ROOT . './include/viewthread_trade.inc.php';
                break;
            case 3:
                include_once DISCUZ_ROOT . './include/viewthread_reward.inc.php';
                break;
            case 4:
                include_once DISCUZ_ROOT . './include/viewthread_activity.inc.php';
                break;
            case 5:
                include_once DISCUZ_ROOT . './include/viewthread_debate.inc.php';
                break;
            case 127:
    ...
    if (empty($postlist)) {
        ...
        $seodescription = ! $thread['price'] ? str_replace(array(
            "\r",
            "\n"
...
{
    global $_DCACHE, $newpostanchor, $numpost, $thisbg, $postcount, $ratelogpids, $onlineauthors, $lastvisit, $thread, $attachpids, $attachtags, $forum, $dateformat, $timeformat, $timeoffset, $userstatusby, $allowgetattach, $ratelogrecord, $showimages, $forum, $discuz_uid, $showavatars, $pagebydesc, $ppp, $ppp2, $ppp3, $firstpid, $threadpay, $sigviewcond, $ordertype;
