    ...
    array(
        ...
        'submenu' => array(
            array(
                ...
                'jswizard&operation=infoside&from=' . $from
            ),
            array(
                'jswizard_infoside_2',
                'jswizard&operation=infoside&sideid=2&from=' . $from
            ),
            array(
                'jswizard_infoside_0',
                'jswizard&operation=infoside&sideid=0&from=' . $from
            )
        )
    ),
    '',
...
if (! $operation) {
    ...
    if (! empty($type)) {
        ...
        showsubmenu('nav_javascript', array(
            array(
                'config',
                'jswizard&operation=config',
                0
            ),
            $infosidemenu,
            array(
                'admin',
                'jswizard' . ($keyp ? '&openkeypre=' . $keypre : ''),
                0
            ),
            array(
                array(
                    'menu' => 'jswizard_addmodule',
                    'submenu' => $addmenu
                ),
                '',
                $type != 'module' ? 1 : 0
            ...
            array(
                'jswizard_module',
                'jswizard&type=module',
                $type != 'module' ? 0 : 1
            ),
            array(
                'import',
                'jswizard&operation=import',
                0
            )
        ));
        if (empty($jskey)) {
    ...
    if (empty($type)) {
        ...
        if (! submitcheck('jsdelsubmit') && ! submitcheck('jsexportsubmit')) {
            ...
            foreach ($jswizard as $key => $jssetting) {
                ...
                showtablerow('', array(
                    ...
                    (in_array($jstypes[$jssetting['type']], array(
                        'custom',
                        'side'
                    )) ? '<b>' . $lang['jswizard_' . $jstypes[$jssetting['type']]] . '</b>' : $lang['jswizard_' . $jstypes[$jssetting['type']]]) . ($jssetting['parameter']['sidestatus'] ? ' &nbsp;(' . $lang['jswizard_for_side'] . ')' : ''),
                    '<a href="' . $BASESCRIPT . '?action=jswizard&function=' . $jstypes[$jssetting['type']] . '&jssetting=' . rawurlencode($key) . '">' . $lang['edit'] . '</a> ' . '<a href="' . $BASESCRIPT . '?action=jswizard&operation=copy&jssetting=' . rawurlencode($key) . '">' . $lang['copy'] . '</a> '
                ));
            }
            echo '</tbody>';
        ...
        $tcheckorderby = array(
            (isset($parameter['orderby']) ? $parameter['orderby'] : 'lastpost') => 'checked'
        );
        for ($i = 0; $i <= 6; $i ++) {
        ...
        $trewardstatus = array(
            intval($parameter['rewardstatus']) => 'checked'
        );
        
        if ($jssubmit && $function == 'threads') {
            
            $jsurl = $jssetting ? $jssetting : "function=$function" . ($parameter['threads_forums'] && ! in_array('all', $parameter['threads_forums']) ? '&fids=' . jsfids($parameter['threads_forums']) : '') . "&sidestatus=$parameter[sidestatus]" . "&maxlength=$parameter[maxlength]" . "&fnamelength=$parameter[fnamelength]" . "&messagelength=$parameter[messagelength]" . "&startrow=$parameter[startrow]" . "&picpre=" . rawurlencode($parameter['picpre']) . "&items=$parameter[items]" . "&tag=" . rawurlencode($parameter[tag]) . '&tids=' . str_replace(',', '_', $parameter['tids']) . ($parameter['keyword'] ? '&keyword=' . rawurlencode($parameter['keyword']) : '') . ($parameter['typeids'] && ! in_array('all', $parameter['typeids']) ? '&typeids=' . jsfids($parameter['typeids']) : '') . ($parameter['sortids'] && ! in_array('all', $parameter['sortids']) ? '&sortids=' . jsfids($parameter['sortids']) : '') . "&special=" . bindec(intval($parameter['special'][1]) . intval($parameter['special'][2]) . intval($parameter['special'][3]) . intval($parameter['special'][4]) . intval($parameter['special'][5]) . intval($parameter['special'][6]) . intval($parameter['special'][0])) . "&rewardstatus=$parameter[rewardstatus]" . "&digest=" . bindec(intval($parameter['digest'][1]) . intval($parameter['digest'][2]) . intval($parameter['digest'][3]) . intval($parameter['digest'][4])) . "&stick=" . bindec(intval($parameter['stick'][1]) . intval($parameter['stick'][2]) . intval($parameter['stick'][3]) . intval($parameter['stick'][4])) . "&recommend=$parameter[recommend]" . "&newwindow=$parameter[newwindow]" . "&threadtype=$parameter[threadtype]" . "&threadsort=$parameter[threadsort]" . "&highlight=$parameter[highlight]" . "&orderby=$parameter[orderby]" . "&hours=" . intval($parameter['hours']) . ($parameter['boardurl'] ? "&boardurl=" . rawurlencode($parameter['boardurl']) : '') . "&jscharset=$parameter[jscharset]" . ($parameter['cachelife'] != '' ? "&cachelife=$parameter[cachelife]" : '') . (! empty($parameter['jstemplate']) ? '&jstemplate=' . rawurlencode($parameter['jstemplate']) : '');
        ...
        showsetting('jswizard_threads_special_reward', array(
            'parameter[rewardstatus]',
            array(
                array(
                    0,
                    $lang['jswizard_threads_special_reward_0']
                ),
                array(
                    1,
                    $lang['jswizard_threads_special_reward_1']
                ),
                array(
                    2,
                    $lang['jswizard_threads_special_reward_2']
                )
            ),
            1
        ), $parameter['rewardstatus'], 'mradio');
        showtagfooter('tbody');
        ...
        if (strtoupper($charset) != 'UTF-8') {
            ...
            showsetting('jswizard_charsetr', array(
                'parameter[jscharset]',
                array(
                    array(
                        0,
            ...
            ), intval($parameter['jscharset']), 'mradio');
        }
        ...
        $fcheckorderby = array(
            (isset($parameter['orderby']) ? $parameter['orderby'] : 'displayorder') => 'checked'
        );
        $parameter['newwindow'] = isset($parameter['newwindow']) ? intval($parameter['newwindow']) : 1;
        $tradionewwindow[$parameter['newwindow']] = 'checked';
        ...
        if ($jssubmit && $function == 'forums') {
            ...
            $jsurl = $jssetting ? $jssetting : "function=$function" . ($parameter['forums_forums'] && ! in_array('all', $parameter['forums_forums']) ? '&fups=' . jsfids($parameter['forums_forums']) : '') . "&startrow=$parameter[startrow]" . "&items=$parameter[items]" . "&newwindow=$parameter[newwindow]" . "&orderby=$parameter[orderby]" . ($parameter['boardurl'] ? "&boardurl=" . rawurlencode($parameter['boardurl']) : '') . "&jscharset=$parameter[jscharset]" . ($parameter['cachelife'] != '' ? "&cachelife=$parameter[cachelife]" : '') . (! empty($parameter['jstemplate']) ? '&jstemplate=' . rawurlencode($parameter['jstemplate']) : '');
        ...
        $mcheckorderby = array(
            (isset($parameter['orderby']) ? $parameter['orderby'] : 'credits') => 'checked'
        );
        $parameter['newwindow'] = isset($parameter['newwindow']) ? intval($parameter['newwindow']) : 1;
        $tradionewwindow[$parameter['newwindow']] = 'checked';
        
        if ($jssubmit && $function == 'memberrank') {
            $jsurl = $jssetting ? $jssetting : "function=$function" . "&startrow=$parameter[startrow]" . "&items=$parameter[items]" . "&newwindow=$parameter[newwindow]" . "&extcredit=$parameter[extcredit]" . "&orderby=$parameter[orderby]" . "&hours=" . intval($parameter['hours']) . ($parameter['boardurl'] ? "&boardurl=" . rawurlencode($parameter['boardurl']) : '') . "&jscharset=$parameter[jscharset]" . ($parameter['cachelife'] != '' ? "&cachelife=$parameter[cachelife]" : '') . (! empty($parameter['jstemplate']) ? '&jstemplate=' . rawurlencode($parameter['jstemplate']) : '');
        ...
        if ($jssubmit && $function == 'stats') {
            if ($jssetting) {
                ...
                $jsurl = "function=$function" . ($parameter['boardurl'] ? "&boardurl=" . rawurlencode($parameter['boardurl']) : '') . "&jscharset=$parameter[jscharset]";
                asort($displayorder);
            ...
            $jsurl .= ($parameter['cachelife'] != '' ? "&cachelife=$parameter[cachelife]" : '') . (! empty($parameter['jstemplate']) ? '&jstemplate=' . rawurlencode($parameter['jstemplate']) : '');
        ...
        showsubtitle(array(
            'jswizard_stats_display',
            'display_order',
            'jswizard_stats_display_title',
            'jswizard_stats_display_name'
        ));
        $order = 0;
        ...
        $tcheckorderby = array(
            (isset($parameter['orderby']) ? $parameter['orderby'] : 'dateline') => 'checked'
        );
        for ($i = 1; $i <= 4; $i ++) {
        ...
        showsetting('jswizard_images_isimage', array(
            'parameter[isimage]',
            array(
                array(
                    0,
                    $lang['jswizard_images_isimage_0'],
                    array(
                        'imgsetting' => 'none'
                    )
                ),
                array(
                    1,
                    $lang['jswizard_images_isimage_1'],
                    array(
                        'imgsetting' => ''
                    )
                ),
                array(
                    2,
                    $lang['jswizard_images_isimage_2'],
                    array(
                        'imgsetting' => 'none'
                    )
                )
        ...
        $parameter['module'] = str_replace(array(
            '..',
            '/',
            '\\'
        ), array(
        ...
        if ($jssubmit && $function == 'module' && $parameter['module']) {
            ...
            $jsurl = $jssetting ? $jssetting : "function=$function" . "&module=$parameter[module]" . "&settings=$settingsenc" . ($parameter['boardurl'] ? "&boardurl=" . rawurlencode($parameter['boardurl']) : '') . "&jscharset=$parameter[jscharset]" . ($parameter['cachelife'] != '' ? "&cachelife=$parameter[cachelife]" : '');
            
            if (! $preview) {
                jssavesetting(5);
            }
            $jspreview = $lang['jswizard_innerrequest'] . '<textarea rows="2" style="width: 95%; word-break: break-all" onFocus="this.select()">{eval request(\'' . str_replace("'", "\'", $jskey) . '\');}</textarea><br />' . $lang['jswizard_outerrequest'] . '<textarea rows="2" style="width: 95%; word-break: break-all" onFocus="this.select()">' . dhtmlspecialchars("<script type=\"text/javascript\" src=\"{$boardurl}api/javascript.php?key=" . rawurlencode($jskey) . "\"></script>") . '</textarea><br /><br />' . jspreviewcode($jsurl) . '<br />';
        }
        ...
        if ($parameter['module']) {
            ...
            if (is_array($request_settings)) {
                foreach ($request_settings as $settingvar => $setting) {
                    $varname = in_array($setting[2], array(
                        'mradio',
                        'mcheckbox',
                        'select',
                        'mselect'
                    )) ? array(
                        'parameter[settings][' . $settingvar . ']',
                        $setting[3]
                    ) : 'parameter[settings][' . $settingvar . ']';
        ...
        if ($jssubmit && $function == $type) {
            ...
            $jsurl = $jssetting ? $jssetting : "function=$function" . ($parameter['boardurl'] ? "&boardurl=" . rawurlencode($parameter['boardurl']) : '') . "&jscharset=$parameter[jscharset]" . ($parameter['cachelife'] != '' ? "&cachelife=$parameter[cachelife]" : '') . (! empty($parameter['jstemplate']) ? '&jstemplate=' . rawurlencode($parameter['jstemplate']) : '');
            
            if (! $preview) {
                jssavesetting($type == 'custom' ? - 1 : - 2);
            }
            $jspreview = ($type == 'custom' ? $lang['jswizard_innerrequest'] . '<textarea rows="2" style="width: 95%; word-break: break-all" onFocus="this.select()">{eval request(\'' . str_replace("'", "\'", $jskey) . '\');}</textarea><br />' . $lang['jswizard_outerrequest'] . '<textarea rows="2" style="width: 95%; word-break: break-all" onFocus="this.select()">' . dhtmlspecialchars("<script type=\"text/javascript\" src=\"{$boardurl}api/javascript.php?key=" . rawurlencode($jskey) . "\"></script>") . '</textarea><br />' : '') . jspreviewcode($jsurl) . '<br />';
        }
        
        echo '<div class="colorbox">';
        if ($jspreview && in_array($type, array(
            'custom',
            'side'
        ))) {
            echo '<h4 style="margin-bottom:15px;">' . lang('preview') . '</h4>' . $jspreview;
    ...
    if (! isset($sideid)) {
        ...
        $sides[] = array(
            '-1',
            lang('jswizard_infoside_newproject')
        );
        ...
        showsetting('jswizard_infoside_project', array(
            'infosidenew',
            $sides
        ), rawurlencode($sidekey), 'select', '', 0, '', "onchange=\"if(this.value && this.value != -1) {window.location='$BASESCRIPT?action=jswizard&operation=infoside&sideid=$sideid&from=$from&sidekey='+this.value} else if(this.value == -1) {\$('newproject').style.display = '';} else {\$('newproject').style.display = 'none';}if(this.value) {\$('modulelist').style.display = '';} else {\$('modulelist').style.display = 'none';}\"");
...
function jsforumselect($function)
{
    ...
    if (empty($function) || in_array($function, array(
        'forums',
        'threads',
        'images'
    ))) {
        $forumselect = '<select name="parameter[' . $function . '_forums][]" size="10" multiple="multiple">' . '<option value="all" ' . (is_array($parameter[$function . '_forums']) && in_array('all', $parameter[$function . '_forums']) ? 'selected="selected"' : '') . '> ' . $lang['jswizard_all_forums'] . '</option>' . '<option value="">&nbsp;</option>';
        if ($function == 'forums') {
...
function jssavesetting($type)
{
    ...
    $jswizard = addslashes(serialize(array(
        'url' => $jsurl,
        'parameter' => $parameter,
        'comment' => $comment
    )));
...
function jsgetrequests()
{
    ...
    while ($entry = $requestdir->read()) {
        if (! in_array($entry, array(
            '.',
            '..'
        )) && preg_match("/^[\w\.]+$/", $entry) && substr($entry, - 8) == '.inc.php' && strlen($entry) < 30 && is_file($dir . '/' . $entry)) {
            include $dir . '/' . $entry;
            $requests[$entry] = array(
                $entry,
                $request_name,
                $request_version,
                $request_copyright,
