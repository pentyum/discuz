...
if (empty($operation)) {
    ...
    if (! submitcheck('advsubmit')) {
        ...
        showsubmenu('adv_admin', array(
            ...
            array(
                array(
                    ...
                    'submenu' => array(
                        array(
                            'adv_type_headerbanner',
                            'adv&operation=advadd&type=headerbanner'
                        ),
                        array(
                            'adv_type_footerbanner',
                            'adv&operation=advadd&type=footerbanner'
                        ),
                        array(
                            'adv_type_text',
                            'adv&operation=advadd&type=text'
                        ),
                        array(
                            'adv_type_thread',
                            'adv&operation=advadd&type=thread'
                        ),
                        array(
                            'adv_type_interthread',
                            'adv&operation=advadd&type=interthread'
                        ),
                        array(
                            'adv_type_float',
                            'adv&operation=advadd&type=float'
                        ),
                        array(
                            'adv_type_couplebanner',
                            'adv&operation=advadd&type=couplebanner'
                        ),
                        array(
                            'adv_type_intercat',
                            'adv&operation=advadd&type=intercat'
                        )
        ...
        $starttimecheck = array(
            $starttime => 'selected="selected"'
        );
        $typecheck = array(
...
<input type="submit" class="btn" name="searchsubmit" value="' . lang('search') . '" onclick="if(this.form.title.value==\'' . lang('adv_inputtitle') . '\'){this.form.title.value=\'\'}window.loacation=\'' . $BASESCRIPT . '?action=adv&title=\'+this.form.title.value+\'&starttime=\'+this.form.starttime.value+\'&type=\'+this.form.type.value+\'&orderby=\'+this.form.orderby.value;"> &nbsp;
...
} elseif ($operation == 'advadd' && in_array($type, array(
    'headerbanner',
    'footerbanner',
    'text',
    'thread',
    ...
    if (! submitcheck('advsubmit')) {
        ...
        if ($operation == 'advedit') {
            ...
            $adv['parameters'] = unserialize($adv['parameters']);
            if (in_array($adv['type'], array(
                'footerbanner',
                'thread'
            ))) {
                if ($adv['type'] == 'thread') {
                    $dispchecked = array();
                    foreach ((isset($adv['parameters']['displayorder']) ? explode("\t", $adv['parameters']['displayorder']) : array(
                        '0'
                    )) as $postcount) {
            ...
            $positionchecked = $type == 'intercat' ? array(
                0 => ' selected="selected"'
            ) : array(
                1 => 'checked'
            );
        ...
        $adv['targets'] = $adv['targets'] != '' && $adv['targets'] != 'forum' ? explode("\t", $adv['targets']) : array(
            'all'
        );
        ...
        if ($type == 'intercat') {
            ...
            $targetsselect = '<select name="advnew[targets][]" size="10" multiple="multiple"><option value="all">&nbsp;&nbsp;> ' . $lang['all'] . '</option>' . '<option value="">&nbsp;</option>' . (in_array($type, array(
                'thread',
                'interthread'
            )) ? '' : '<option value="0">&nbsp;&nbsp;> ' . $lang['home'] . '</option>') . (in_array($type, array(
                'headerbanner',
                'footerbanner'
            )) ? '</option><option value="register">&nbsp;&nbsp;> ' . $lang['adv_register'] . '</option>' . '</option><option value="redirect">&nbsp;&nbsp;> ' . $lang['adv_jump'] . '</option>' . '</option><option value="archiver">&nbsp;&nbsp;> Archiver</option>' : '') . '</option>' . forumselect(in_array($type, array(
                'headerbanner',
                'footerbanner',
                'text',
                'float',
                'couplebanner'
            )) ? TRUE : FALSE, 0, 0, TRUE) . '</select>';
        ...
        if ($adv['type'] == 'thread') {
            showsetting('adv_edit_position_thread', array(
                'advnew[position]',
                array(
                    array(
                        1,
                        $lang['adv_thread_down']
                    ),
                    array(
                        2,
                        $lang['adv_thread_up']
                    ...
                    array(
                        3,
                        $lang['adv_thread_right']
                    )
                ),
                TRUE
            ), $adv['parameters']['position'], 'mradio');
            showsetting('adv_edit_display_position', '', '', $dispselect);
        } elseif ($adv['type'] == 'footerbanner') {
            showsetting('adv_edit_position_footerbanner', array(
                'advnew[position]',
                array(
                    array(
                        1,
        ...
        } elseif (($advnew['style'] == 'code' && ! $advnew['code']['html']) || ($advnew['style'] == 'text' && (! $advnew['text']['title'] || ! $advnew['text']['link'])) || ($advnew['style'] == 'image' && (! $advnew['image']['url'] || ! $advnew['image']['link'])) || ($advnew['style'] == 'flash' && (! $advnew['flash']['url'] || ! $advnew['flash']['width'] || ! $advnew['flash']['height']))) {
        ...
        if (is_array($advnew['targets'])) {
            ...
            foreach ($advnew['targets'] as $target) {
                if (in_array($type, array(
                    ...
                    'float',
                    'couplebanner'
                )) && in_array($target, $gids)) {
                    $targetsarray[] = 'gid' . $target;
                } elseif ($target == 'all') {
                    $targetsarray = in_array($type, array(
                        'thread',
                        'interthread'
                    )) ? array(
                        'forum'
                    ...
                    break;
                } elseif (in_array($target, array(
                    'register',
                    'redirect',
                    'archiver'
                )) || preg_match("/^\d+$/", $target) && ($target == 0 || in_array($_DCACHE['forums'][$target]['type'], array(
                    'forum',
                    'sub'
                )))) {
                    $targetsarray[] = $target;
        ...
        $advnew['parameters'] = addslashes(serialize(array_merge(array(
            'style' => $advnew['style']
        ), $advnew['style'] == 'code' ? array() : $advnew[$advnew['style']], array(
            'html' => $advnew['code']
        ), array(
    ...
    if (! submitcheck('configsubmit')) {
        ...
        showtableheader();
        showsetting('adv_config_mode', array(
            'admodenew',
            array(
                array(
                    0,
                    lang('adv_config_mode_0')
                ),
                array(
                    1,
                    lang('adv_config_mode_1')
                )
            )
        ), $admode, 'select');
...
function showtargetlink($target)
{
    ...
    return substr($target, 0, 3) == 'gid' ? '<a href="' . $indexname . '?gid=' . substr($target, 3) . '" target="_blank">' . $_DCACHE['forums'][substr($target, 3)]['name'] . '</a>' : ($target == 'register' ? '<a href="' . $regname . '" target="_blank">' . lang('adv_register') . '</a>' : ($target == 'redirect' ? lang('adv_jump') : ($target == 'archiver' ? '<a href="archiver/" target="_blank">Archiver</a>' : ($target ? '<a href="forumdisplay.php?fid=' . $target . '" target="_blank">' . $_DCACHE['forums'][$target]['name'] . '</a>' : '<a href="' . $indexname . '" target="_blank">' . lang('home') . '</a>'))));
}
