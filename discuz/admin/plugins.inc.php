...
if (! $operation) {
    ...
    if (! submitcheck('submit')) {
        ...
        while ($plugin = $db->fetch_array($query)) {
            ...
            showtablerow('style="height:20px"', array(
                'class="td25"',
                ($plugin['available'] ? 'class="bold"' : ''),
                'class="td25"',
                '',
                '',
                'class="td28 td23"',
                ...
                "<a href=\"$BASESCRIPT?action=plugins&operation=delete&pluginid=$plugin[pluginid]\" class=\"act\">$lang[plugins_config_uninstall]</a>&nbsp;" . "<a href=\"$BASESCRIPT?action=plugins&operation=edit&pluginid=$plugin[pluginid]\" class=\"act\">$lang[plugins_editlink]</a>"
            ));
        ...
        while ($entry = $pluginsdir->read()) {
            if (! in_array($entry, array(
                '.',
                '..'
            )) && is_dir($plugindir . '/' . $entry) && ! in_array($entry . '/', $installsdir)) {
    ...
    if ($hookscript) {
        foreach ($hookscript as $script => $value) {
            ...
            $funcs = array();
            $scripttitle = in_array($script, array(
                'index',
                'profile',
                'global'
            )) ? $lang['plugins_hooklist_script_' . $script] : (file_exists(DISCUZ_ROOT . './' . $script . '.php') ? $script . '.php' : $script);
            showsubtitle(array(
                $scripttitle,
                ''
            ));
    ...
    if (! submitcheck('importsubmit') && ! isset($dir)) {
        ...
        if (! isset($dir)) {
            ...
            if (empty($license) && $pluginarray['license']) {
                ...
                echo '<div class="infobox"><h4 class="infotitle2">' . $pluginarray['plugin']['name'] . ' ' . $pluginarray['plugin']['version'] . ' ' . $lang['plugins_import_license'] . '</h4><div style="text-align:left;line-height:25px;">' . $pluginarray['license'] . '</div><br /><br /><center>' . '<button onclick="location.href=\'' . $BASESCRIPT . '?action=plugins&operation=import&dir=' . $dir . '&installtype=' . $installtype . '&license=yes\'">' . $lang['plugins_import_agree'] . '</button>&nbsp;&nbsp;' . '<button onclick="location.href=\'' . $BASESCRIPT . '?action=plugins\'">' . $lang['plugins_import_pass'] . '</button></center></div>';
                exit();
            }
        }
    ...
    if (! $mod) {
        ...
        $submenuitem = array(
            array(
                'plugins_home',
                "plugins&operation=config&pluginid=$pluginid&anchor=home",
                0
            )
        );
        if ($pluginvars) {
            $submenuitem[] = array(
                'config',
                "plugins&operation=config&pluginid=$pluginid&anchor=vars",
                0
            );
        }
    }
    if (is_array($plugin['modules'])) {
        foreach ($plugin['modules'] as $module) {
            if ($module['type'] == 3) {
                $submenuitem[] = array(
                    $module['menu'],
                    "plugins&operation=config&identifier=$plugin[identifier]&mod=$module[name]",
                    $mod == $module['name'],
                    ! $mod ? 1 : 0
                );
            }
                ...
                foreach ($varsnew as $variable => $value) {
                    if (isset($pluginvars[$variable])) {
                        if ($pluginvars[$variable]['type'] == 'number') {
                            $value = (float) $value;
                        } elseif (in_array($pluginvars[$variable]['type'], array(
                            'forums',
                            'groups'
                        ))) {
    ...
    if (! submitcheck('editsubmit')) {
        ...
        showsubmenuanchors($lang['plugins_edit'] . ' - ' . $plugin['name'], array(
            ...
            array(
                'plugins_config_module',
                'modules',
                $anchor == 'modules'
            ),
            array(
                'plugins_config_hooks',
                'hooks',
                $anchor == 'hooks'
            ),
            array(
            ...
            array(
                'export',
                'plugins&operation=export&pluginid=' . $plugin['pluginid'],
                0,
                1
            )
        ));
        ...
        showsubtitle(array(
            '',
            'plugins_edit_modules_type',
            'plugins_edit_modules_name',
            'plugins_edit_modules_menu',
            'plugins_edit_modules_menu_url',
            'plugins_edit_modules_adminid',
            'display_order'
        ));
        ...
        if (is_array($plugin['modules'])) {
            foreach ($plugin['modules'] as $moduleid => $module) {
                ...
                $adminidselect = array(
                    $module['adminid'] => 'selected'
                );
                $includecheck = empty($val['include']) ? $lang['no'] : $lang['yes'];
                
                $typeselect = '<optgroup label="' . lang('plugins_edit_modules_type_g1') . '">' . '<option h="1111" e="inc" value="1"' . ($module['type'] == 1 || $module['type'] == 2 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_1') . '</option>' . '<option h="1111" e="inc" value="5"' . ($module['type'] == 5 || $module['type'] == 6 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_5') . '</option>' . '<option h="1111" e="inc" value="7"' . ($module['type'] == 7 || $module['type'] == 8 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_7') . '</option>' . '<option h="1111" e="inc" value="9"' . ($module['type'] == 9 || $module['type'] == 10 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_9') . '</option>' . '<option h="1001" e="inc" value="14"' . ($module['type'] == 14 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_14') . '</option>' . '<option h="1001" e="inc" value="15"' . ($module['type'] == 15 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_15') . '</option>' . '<option h="1001" e="inc" value="16"' . ($module['type'] == 16 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_16') . '</option>' . '<option h="1001" e="inc" value="3"' . ($module['type'] == 3 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_3') . '</option>' . '</optgroup>' . '<optgroup label="' . lang('plugins_edit_modules_type_g2') . '">' . '<option h="0000" e="inc" value="13"' . ($module['type'] == 13 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_13') . '</option>' . '<option h="0011" e="inc" value="4"' . ($module['type'] == 4 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_4') . '</option>' . '<option h="0011" e="class" value="11"' . ($module['type'] == 11 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_11') . '</option>' . '<option h="0001" e="class" value="12"' . ($module['type'] == 12 ? ' selected="selected"' : '') . '>' . lang('plugins_edit_modules_type_12') . '</option>' . '</optgroup>';
                showtablerow('', array(
                    'class="td25"',
                    'class="td28"'
                ), array(
        ...
        showtablerow('', array(
            ...
            '<span id="a_n"><select name="newadminid">' . '<option value="0">' . lang('usergroups_system_0') . '</option>' . '<option value="1" selected>' . lang('usergroups_system_1') . '</option>' . '<option value="2">' . lang('usergroups_system_2') . '</option>' . '<option value="3">' . lang('usergroups_system_3') . '</option>' . '</select></span>',
        ...
        showsubtitle(array(
            ...
            'available',
            'plugins_hooks_title',
            'plugins_hooks_callback',
            'plugins_hooks_description',
            ''
        ));
        ...
        if ($type == 'common') {
            ...
            foreach ($modulesnew as $key => $module) {
                $namekey = in_array($module['type'], array(
                    11,
                    12
                )) ? 1 : 0;
    ...
    if (! submitcheck('hooksubmit')) {
        ...
        showsubmenu($lang['plugins_edit'] . ' - ' . $pluginhook['name'], array(
            ...
            array(
                'config',
                'plugins&operation=edit&pluginid=' . $pluginid . '&anchor=config',
                0
            ),
            array(
                'plugins_config_module',
                'plugins&operation=edit&pluginid=' . $pluginid . '&anchor=modules',
                0
            ),
            array(
                'plugins_config_hooks',
                'plugins&operation=edit&pluginid=' . $pluginid . '&anchor=hooks',
                1
            ),
            array(
                'plugins_config_vars',
                'plugins&operation=edit&pluginid=' . $pluginid . '&anchor=vars',
                0
            ),
            array(
                'export',
                'plugins&operation=export&pluginid=' . $pluginid,
                0
            )
        ));
