    ...
    if (! submitcheck('groupsubmit')) {
        ...
        while ($group = $db->fetch_array($query)) {
            if ($group['type'] == 'member') {
                ...
                $membergroup .= showtablerow('', array(
                    'class="td25"',
                    '',
                    'class="td28"',
                    'class=td28'
        ...
        foreach ($sgroups as $group) {
            ...
            $specialgroup .= showtablerow('', array(
                'class="td25"',
                '',
                'class="td28"'
            ), array(
            ...
            $specialgroup .= showtablerow('', array(
                '',
                'colspan="6" id="sgroup_' . $group['groupid'] . '" style="display: none"'
            ), array(
                '',
                ''
            ), TRUE);
...
EOT;
    ...
    if (! submitcheck('detailsubmit') && ! submitcheck('saveconfigsubmit')) {
        ...
        $gselect = '<span id="ugselect" class="right popupmenu_dropmenu" onmouseover="showMenu({\'ctrlid\':this.id,\'pos\':\'34\'});$(\'ugselect_menu\').style.top=(parseInt($(\'ugselect_menu\').style.top)-document.documentElement.scrollTop)+\'px\'">' . $lang['usergroups_switch'] . '<em>&nbsp;&nbsp;</em></span>' . '<div id="ugselect_menu" class="popupmenu_popup" style="display:none">' . '<em>' . $lang['usergroups_member'] . '</em>' . $grouplist['member'] . '<br />' . ($grouplist['special'] ? '<em>' . $lang['usergroups_special'] . '</em>' . $grouplist['special'] . '<br />' : '') . ($grouplist['specialadmin'] ? '<em>' . $lang['usergroups_specialadmin'] . '</em>' . $grouplist['specialadmin'] . '<br />' : '') . '<em>' . $lang['usergroups_system'] . '</em>' . $grouplist['system'] . '</div>';
        ...
        showsubmenuanchors(lang('usergroups_edit') . ' - ' . $group['grouptitle'], array(
            array(
                'usergroups_edit_basic',
                'basic',
                $anchor == 'basic'
            ),
            $group['type'] == 'special' && $group['radminid'] < 1 ? array(
                'usergroups_edit_system',
                'system',
                $anchor == 'system'
            ) : array(),
            array(
                'usergroups_edit_special',
                'special',
                $anchor == 'special'
            ),
            array(
                'usergroups_edit_post',
                'post',
                $anchor == 'post'
            ),
            array(
                'usergroups_edit_attach',
                'attach',
                $anchor == 'attach'
            ),
            array(
                'usergroups_edit_magic',
                'magic',
                $anchor == 'magic'
            ),
            array(
                'usergroups_edit_invite',
                'invite',
                $anchor == 'invite'
            ),
            array(
                'usergroups_edit_credit',
                'credit',
                $anchor == 'credit'
            )
        ), $gselect);
        ...
        showsetting('usergroups_edit_basic_search', array(
            'allowsearchnew',
            array(
                array(
                    0,
                    $lang['usergroups_edit_basic_search_disable']
                ),
                array(
                    1,
                    $lang['usergroups_edit_basic_search_thread']
                ),
                array(
                    2,
                    $lang['usergroups_edit_basic_search_post']
                )
            )
        ), $group['allowsearch'], 'mradio');
        showsetting('usergroups_edit_basic_reasonpm', array(
            'reasonpmnew',
            array(
                array(
                    0,
                    $lang['usergroups_edit_basic_reasonpm_none']
                ),
                array(
                    1,
                    $lang['usergroups_edit_basic_reasonpm_reason']
                ),
                array(
                    2,
                    $lang['usergroups_edit_basic_reasonpm_pm']
                ),
                array(
                    3,
                    $lang['usergroups_edit_basic_reasonpm_both']
                )
            )
        ...
        showsetting('usergroups_edit_post_direct', array(
            'allowdirectpostnew',
            array(
                array(
                    0,
                    $lang['usergroups_edit_post_direct_none']
                ),
                array(
                    1,
                    $lang['usergroups_edit_post_direct_reply']
                ),
                array(
                    2,
                    $lang['usergroups_edit_post_direct_thread']
                ),
                array(
                    3,
                    $lang['usergroups_edit_post_direct_all']
                )
            )
        ), $group['allowdirectpost'], 'mradio');
        showsetting('usergroups_edit_post_url', array(
            'allowposturlnew',
            array(
                array(
                    0,
                    $lang['usergroups_edit_post_url_banned']
                ),
                array(
                    1,
                    $lang['usergroups_edit_post_url_mod']
                ),
                array(
                    2,
                    $lang['usergroups_edit_post_url_unhandle']
                ),
                array(
                    3,
                    $lang['usergroups_edit_post_url_enable']
                )
            )
        ...
        showsetting('usergroups_edit_magic_permission', array(
            'allowmagicsnew',
            array(
                array(
                    0,
                    $lang['usergroups_edit_magic_unallowed']
                ),
                array(
                    1,
                    $lang['usergroups_edit_magic_allow']
                ),
                array(
                    2,
                    $lang['usergroups_edit_magic_allow_and_pass']
                )
            )
        ...
        if ($group['radminid']) {
            if ($group['radminid'] == 3) {
                ...
                echo '<input name="exemptnew[2]" type="hidden" value="1" /><input name="exemptnew[3]" type="hidden" value="1" /><input name="exemptnew[4]" type="hidden" value="1" />' . '<input name="exemptnew[5]" type="hidden" value="1" /><input name="exemptnew[6]" type="hidden" value="1" /><input name="exemptnew[7]" type="hidden" value="1" />';
            }
        } else {
        ...
        showsubtitle(array(
            ...
            'credits_id',
            'credits_title',
            'usergroups_edit_credit_rate_min',
            'usergroups_edit_credit_rate_max',
            'usergroups_edit_credit_rate_mrpd'
        ));
        for ($i = 1; $i <= 8; $i ++) {
            if (isset($extcredits[$i])) {
                echo '<tr><td><input class="checkbox" type="checkbox" name="raterangenew[' . $i . '][allowrate]" value="1" ' . (empty($raterangearray[$i]) ? '' : 'checked') . '></td>' . '<td>extcredits' . $i . '</td>' . '<td>' . $extcredits[$i]['title'] . '</td>' . '<td><input type="text" class="txt" name="raterangenew[' . $i . '][min]" size="3" value="' . $raterangearray[$i]['min'] . '"></td>' . '<td><input type="text" class="txt" name="raterangenew[' . $i . '][max]" size="3" value="' . $raterangearray[$i]['max'] . '"></td>' . '<td><input type="text" class="txt" name="raterangenew[' . $i . '][mrpd]" size="3" value="' . $raterangearray[$i]['mrpd'] . '"></td></tr>';
            }
        }
        ...
        if ($group['type'] == 'special' && $group['radminid'] > 0) {
            ...
            $radminidnew = in_array($group['groupid'], array(
                1,
                2,
                3
            )) ? $group['groupid'] : 0;
            ...
            foreach ($raterangenew as $key => $rate) {
                if ($key >= 1 && $key <= 8 && $rate['allowrate']) {
                    ...
                    if (! $rate['mrpd'] || $rate['max'] <= $rate['min'] || $rate['mrpd'] < max(abs($rate['min']), abs($rate['max']))) {
                        ...
                        $raterangenew[$key] = implode("\t", array(
                            $key,
                            $rate['min'],
                            $rate['max'],
                            $rate['mrpd']
