...
if (! $operation) {
    ...
    if (! submitcheck('groupsubmit')) {
        ...
        if ($grouptitlenew && in_array($radminidnew, array(
            1,
            2,
            3
        ))) {
    ...
    $actioncat = array(
        'admingroups_edit_action_cat_basesetting' => array(
            array(
                '_readonly',
                'settings',
                'templates'
        ...
        'admingroups_edit_action_cat_resourcesetting' => array(
            array(
                'forums',
                'members'
            ),
        ...
        'admingroups_edit_action_cat_resourceadmin' => array(
            array(
                'moderate'
            ),
            array(
        ...
        'admingroups_edit_action_cat_extraadmin' => array(
            array(
                'ec',
                'plugins',
                'adv',
        ...
        'admingroups_edit_action_cat_otheradmin' => array(
            array(
                'misc',
                'tools'
            ),
        ...
        'admingroups_edit_action_cat_maint' => array(
            array(
                'db',
                'logs'
            ),
    ...
    if (! $submitcheck && empty($gset)) {
        ...
        $anchor = $group['radminid'] == 1 ? (in_array($anchor, array(
            'admincpperm',
            'threadperm',
            'postperm',
            'modcpperm',
            'otherperm'
        )) ? $anchor : 'admincpperm') : (in_array($anchor, array(
            'threadperm',
            'postperm',
            'modcpperm',
            'otherperm'
        ...
        $anchorarray = array(
            $group['radminid'] == 1 ? array(
                'admingroups_edit_admincpperm',
                'admincpperm',
                $anchor == 'admincpperm'
            ) : array(),
            array(
                'admingroups_edit_threadperm',
                'threadperm',
                $anchor == 'threadperm'
            ),
            array(
                'admingroups_edit_postperm',
                'postperm',
                $anchor == 'postperm'
            ),
            array(
                'admingroups_edit_modcpperm',
                'modcpperm',
                $anchor == 'modcpperm'
            ),
            array(
                'admingroups_edit_otherperm',
                'otherperm',
                $anchor == 'otherperm'
            )
        );
        ...
        showsetting('admingroups_edit_stick_thread', array(
            'allowstickthreadnew',
            array(
                array(
                    0,
                    $lang['admingroups_edit_stick_thread_none']
                ),
                array(
                    1,
                    $lang['admingroups_edit_stick_thread_1']
                ),
                array(
                    2,
                    $lang['admingroups_edit_stick_thread_2']
                ),
                array(
                    3,
                    $lang['admingroups_edit_stick_thread_3']
                )
            )
        ), $group['allowstickthread'], 'mradio');
        showsetting('admingroups_edit_digest_thread', array(
            'allowdigestthreadnew',
            array(
                array(
                    0,
                    $lang['admingroups_edit_digest_thread_none']
                ),
                array(
                    1,
                    $lang['admingroups_edit_digest_thread_1']
                ),
                array(
                    2,
                    $lang['admingroups_edit_digest_thread_2']
                ),
                array(
                    3,
                    $lang['admingroups_edit_digest_thread_3']
                )
            )
        ...
        if (! empty($gset)) {
            ...
            while ($group = $db->fetch_array($query)) {
                ...
                if (! in_array($group['admingid'], $gsetnew)) {
                    ...
                    $group['disabledactions'] = array_diff($group['disabledactions'], array(
                        $gset
                    ));
                }
