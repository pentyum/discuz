...
if (! $operation) {
    ...
    if (is_array($data['Data']))
        foreach ($data['Data'] as $row) {
            ...
            echo '<tr><th colspan="3" class="partition">' . ($row['Time'] != '' ? '<div class="right">' . $row['Time'] . '</div>' : '') . '<a href="' . $row['Url'] . '" target="_blank">' . ($row['Greenplugin'] ? '<img class="vmiddle" title="' . $lang['addons_greenplugin'] . '" src="images/admincp/greenplugin.gif" /> ' : '') . $row['Name'] . ($row['Version'] != '' ? ' ' . $row['Version'] : '') . '</a></th></tr>' . '<tr><td width="110">' . ($row['Thumb'] != '' ? '<a href="' . $row['Url'] . '" target="_blank"><img onerror="this.src=\'images/common/none.gif\'" src="' . $row['Thumb'] . '" width="100" /></a>' : '') . '</td>' . '<td width="90%" class="lineheight" valign="top">' . ($row['Charset'] != '' ? $lang['addons_charset'] . implode(', ', $Charset) . '<br />' : '') . ($row['Description'] != '' ? nl2br($row['Description']) : '') . '</td></tr>';
            if ($count == 20) {
                break;
            }
        }
    ...
    if (empty($providerkey)) {
        ...
        if (is_array($data) && $data) {
            ...
            echo '<tr><td>';
            foreach ($data as $row) {
                echo '<div class="hover" style="float:left;width:20%"><div style="text-align:center"><a href="' . $BASESCRIPT . '?action=addons&operation=add&providerkey=' . $row['key'] . '">' . ($row['logo'] ? '<img width="100" height="50" src="' . $row['logo'] . '" />' : '<img width="100" height="50" src="images/common/none.gif" />') . '</a><br /><a href="' . $BASESCRIPT . '?action=addons&operation=add&providerkey=' . $row['key'] . '">' . $row['sitename'] . '</a></div></div>';
            }
            echo '</td></tr>';
        } else {
...
function showproviderinfo($addon, $simple)
{
    ...
    echo (! $simple ? '<tr><th colspan="3" class="partition"><a href="' . $BASESCRIPT . '?action=addons&operation=list&provider=' . $addon['key'] . '">' . $addon['title'] . '</a></th></tr>' : '') . '<tr><td width="110"><a href="' . $BASESCRIPT . '?action=addons&operation=list&provider=' . $addon['key'] . '"><img onerror="this.src=\'images/common/none.gif\'" src="' . $addon['logo'] . '" width="100" height="50" /></a></td>' . '<td valign="top">' . nl2br($addon['description']) . '<br />' . lang('addons_provider') . '<a href="' . $addon['siteurl'] . '" target="_blank">' . $addon['sitename'] . '</a><br />' . lang('addons_contact') . $contact . '</td>' . (! $simple ? '<td align="right" width="50">' . (! $addon['system'] ? '<a href="' . $BASESCRIPT . '?action=addons&operation=remove&provider=' . $addon['key'] . '" onclick="return confirm(\'' . lang('addons_delete_confirm') . '\')">' . lang('delete') . '</a>' : '') . '&nbsp;</td>' : '') . '</tr>';
    showtablefooter();
    if ($simple) {
        echo '</div>';
    }
