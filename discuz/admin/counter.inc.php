...
if (submitcheck('forumsubmit', 1)) {
    ...
    if ($sortids[$cursort] && $optionvalues[$sortids[$cursort]]) {
        ...
        foreach ($optionids[$sortid] as $optionid => $identifier) {
            if (! $tables[$identifier] && (in_array($options[$identifier], array(
                'radio',
                'select',
                'number'
            )) || $search[$optionid])) {
                $fieldname = $identifier;
                if (in_array($options[$identifier], array(
                    'radio',
                    'select'
                ))) {
    ...
    if ($processed) {
        cpmsg("$lang[counter_special_arrange]:(��{$cursort}��/��" . count($sortids) . "��)", $nextlink, 'loading');
    } else {
