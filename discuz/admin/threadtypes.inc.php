...
if (! $operation) {
    ...
    if (! submitcheck('typesubmit')) {
        ...
        while ($type = $db->fetch_array($query)) {
            $threadtypes .= showtablerow('', array(
                'class="td25"',
                'class="td28"',
                '',
                'class="td29"',
                'title="' . lang('forums_threadtypes_forums_comment') . '"',
                'class="td25"'
            ), array(
...
<tr>
	<td class="td25"></td>
	<td colspan="4"><div>
			<a href="###" onclick="addrow(this, 0)" class="addtr"><?=$lang['threadtype_infotypes_add']?></a>
		</div></td>
<?
...
<?
    ...
    if (! submitcheck('typeoptionsubmit')) {
        ...
        while ($option = $db->fetch_array($query)) {
            $classoptions[] = array(
                $option[title],
                "threadtypes&operation=typeoption&classid=$option[optionid]",
                $classid == $option[optionid]
            );
        }
        ...
        if ($classid) {
            ...
            while ($option = $db->fetch_array($query)) {
                ...
                $typeoptions .= showtablerow('', array(
                    'class="td25"',
                    'class="td28"'
                ), array(
...
EOT;
    ...
    if (! submitcheck('sortdetailsubmit') && ! submitcheck('sortpreviewsubmit')) {
        ...
        foreach ($showoption as $optionid => $option) {
            $sortoptions .= showtablerow('id="optionid' . $optionid . '"', array(
                'class="td25"',
                'class="td28 td23"'
            ), array(
        ...
        showsubtitle(array(
            '<input type="checkbox" name="chkall" id="chkall" class="checkbox" onclick="checkAll(\'prefix\', this.form,\'delete\')" /><label for="chkall">' . lang('del') . '</label>',
            'display_order',
            'available',
            'name',
            'type',
            'required',
            'unchangeable',
            'threadtype_infotypes_search',
            'threadtype_infotypes_show',
            '',
            '',
            ''
        ));
...
<div class="colorbox">
	...
	<textarea cols="100" rows="5" id="typetemplate" name="typetemplate"
		style="width: 95%;" onkeyup="textareasize(this)"><?=$threadtype['template']?></textarea>
	<br />
	<br />
	<h4 style="margin-bottom: 15px;"><?=$threadtype['name']?> - <?=$lang['threadtype_infotypes_stemplate']?></h4>
	<textarea cols="100" rows="5" id="stypetemplate" name="stypetemplate"
		style="width: 95%;" onkeyup="textareasize(this)"><?=$threadtype['stemplate']?></textarea>
	<br />
	<br /> <b><?=$lang['threadtype_infotypes_template']?>:</b>
...
<input type="submit" class="btn" name="sortdetailsubmit"
		value="<?=$lang['submit']?>"> &nbsp; <input type="submit" class="btn"
		name="sortpreviewsubmit"
		value="<?=$lang['threadtype_infotypes_template_preview']?>">
</div>
            ...
            if (is_array($addoption)) {
                ...
                if ($db->num_rows($query) != 1) {
                    ...
                    foreach ($addoption as $optionid => $option) {
                        ...
                        if (in_array($insertoptionid[$optionid]['type'], array(
                            'radio',
                            'select',
                            'number'
                        )) || $search[$optionid]) {
                    ...
                    foreach ($addoption as $optionid => $option) {
                        $identifier = $insertoptionid[$optionid]['identifier'];
                        if (! $tables[$identifier] && (in_array($insertoptionid[$optionid]['type'], array(
                            'radio',
                            'select',
                            'number'
                        )) || $search[$optionid])) {
