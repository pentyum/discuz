...
if (! $operation) {
    ...
    showsubmenu('nav_recyclebin', array(
        ...
        array(
            'search',
            'recyclebin&operation=search',
            0
        ),
        array(
            'clean',
            'recyclebin&operation=clean',
            0
        )
    ));
    ...
    if (! submitcheck('delsubmit') && ! submitcheck('undelsubmit')) {
        ...
        showsubtitle(array(
            '',
            'thread',
            'recyclebin_list_thread',
            'recyclebin_list_author',
            'recyclebin_list_status',
            'recyclebin_list_lastpost',
            'recyclebin_list_operation'
        ));
    ...
    if (! submitcheck('rbsubmit')) {
        ...
        $forumselect = '<select name="inforum"><option value="">&nbsp;&nbsp;> ' . $lang['select'] . '</option>' . '<option value="">&nbsp;</option>' . forumselect(FALSE, 0, 0, TRUE) . '</select>';
...
EOT;
        ...
        showsetting('recyclebin_search_post_time', array(
            'pstarttime',
            'pendtime'
        ), array(
            $pstarttime,
        ...
        showsetting('recyclebin_search_mod_time', array(
            'mstarttime',
            'mendtime'
        ), array(
            $mstarttime,
        ...
        if (submitcheck('searchsubmit')) {
            ...
            while ($thread = $db->fetch_array($query)) {
                ...
                showtablerow("id=\"mod_$thread[tid]_row1\"", array(
                    'rowspan="3" class="rowform threadopt" style="width:80px;"',
                    'class="threadtitle"'
                ), array(
