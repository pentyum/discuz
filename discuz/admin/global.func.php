...
{
    $user = empty($user) ? array(
        'uid' => $GLOBALS['discuz_uid'],
        'adminid' => $GLOBALS['adminid'],
        'username' => $GLOBALS['discuz_userss']
    ) : $user;
...
function shownav($header = '', $menu = '', $nav = '')
{
    ...
    echo '<script type="text/JavaScript">if(parent.$(\'admincpnav\')) parent.$(\'admincpnav\').innerHTML=\'' . lang('nav_' . ($header ? $header : 'index')) . ($menu ? '&nbsp;&raquo;&nbsp;' . lang($menu) : '') . ($nav ? '&nbsp;&raquo;&nbsp;' . lang($nav) : '') . '\';' . 'if(parent.$(\'add2custom\')) parent.$(\'add2custom\').innerHTML=' . ($customtitle ? '\'<a href="' . $BASESCRIPT . '?action=misc&operation=custommenu&do=add&title=' . $customtitle . '&url=' . cpurl() . '" target="main"><img src="images/admincp/btn_add2menu.gif" title="' . lang('custommenu_add') . '" width="19" height="18" /></a>\';' : '\'\';') . ($customtitle ? 'if(parent.$(\'custombar_add\')) parent.$(\'custombar_add\').innerHTML=' . ($customtitle ? '\'<span onclick="ajaxget(\\\'' . $BASESCRIPT . '?action=misc&operation=custombar&title=' . $customtitle . '&url=' . cpurl() . '\\\', \\\'custombar\\\', \\\'\\\', \\\'\\\', \\\'\\\', function () { top.custombar_resize();});" title="' . lang('custombar_add_tips') . '" />&nbsp;&nbsp;&nbsp;&nbsp;' . lang('custombar_add') . '</span>\';' : '\'\';') : '') . 'top.custombar_resize();' . '</script>';
}
...
{
    global $BASESCRIPT;
    echo '<form name="' . $name . '" method="post" action="' . $BASESCRIPT . '?action=' . $action . '" id="' . $name . '"' . ($extra == 'enctype' ? ' enctype="multipart/form-data"' : " $extra") . '>' . '<input type="hidden" name="formhash" value="' . FORMHASH . '" />' . '<input type="hidden" id="formscrolltop" name="scrolltop" value="" />' . '<input type="hidden" name="anchor" value="' . htmlspecialchars($GLOBALS['anchor']) . '" />';
}
...
function showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '')
{
    ...
    if ($type == 'radio') {
        ...
        $s .= '<ul onmouseover="altStyle(this);">' . '<li' . ($check['true'] ? ' class="checked"' : '') . '><input class="radio" type="radio" name="' . $varname . '" value="1" ' . $check['true'] . $check['hidden1'] . $check['disabled'] . '>&nbsp;' . lang('yes') . '</li>' . '<li' . ($check['false'] ? ' class="checked"' : '') . '><input class="radio" type="radio" name="' . $varname . '" value="0" ' . $check['false'] . $check['hidden0'] . $check['disabled'] . '>&nbsp;' . lang('no') . '</li>' . '</ul>';
    ...
    showtablerow('class="noborder"', array(
        'class="vtop rowform"',
        'class="vtop tips2"'
    ), array(
        $s,
        ($comment ? $comment : lang($setname . '_comment', 0)) . ($type == 'textarea' ? '<br />' . lang('tips_textarea') : '') . ($disabled ? '<br /><span class="smalltxt" style="color:#FF0000">' . lang($setname . '_disabled', 0) . '</span>' : NULL)
    ));
    if ($hidden) {
...
{
    $str = '<tr>';
    $str .= $name && in_array($before, array(
        'del',
        'select_all',
        'td'
    )) ? '<td class="td25">' . ($before != 'td' ? '<input type="checkbox" name="chkall" id="chkall" class="checkbox" onclick="checkAll(\'prefix\', this.form, \'delete\')" /><label for="chkall">' . lang($before) : '') . '</label></td>' : '';
    $str .= '<td colspan="15">';
    $str .= $floatright ? '<div class="cuspages right">' . $floatright . '</div>' : '';
    $str .= '<div class="fixsel">';
    $str .= $before && ! in_array($before, array(
        'del',
        'select_all',
        'td'
    )) ? $before . ' &nbsp;' : '';
...
{
    showsetting('import_type', array(
        'importtype',
        array(
            array(
                'file',
                lang('import_type_file'),
                array(
                    'importfile' => '',
                    'importtxt' => 'none'
                )
...
function exportdata($name, $filename, $data)
{
    ...
    $filename = strtolower(str_replace(array(
        '!',
        ' '
    ), array(
        '',
