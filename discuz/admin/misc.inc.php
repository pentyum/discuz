...
<?
    ...
    if (! submitcheck('bbcodessubmit') && ! $edit) {
        ...
        if (! submitcheck('editsubmit')) {
            ...
            $promptnew = trim(str_replace(array(
                "\t",
                "\r",
                "\n"
            ), array(
    ...
    if (! submitcheck('typesubmit')) {
        ...
        while ($type = $db->fetch_array($query)) {
            $attachtypes .= showtablerow('', array(
                'class="td25"',
                'class="td24"'
            ), array(
...
<?
        ...
        showtableheader();
        showtablerow('class="partition"', array(
            'class="td25"'
        ), array(
            '',
            lang('misc_attachtype_ext'),
            lang('misc_attachtype_maxsize')
        ));
        echo $attachtypes;
    ...
    if (empty($edit) && empty($run)) {
        ...
        if (! submitcheck('cronssubmit')) {
            ...
            while ($cron = $db->fetch_array($query)) {
                ...
                showtablerow('', array(
                    ...
                    $cron[lastrun],
                    $cron[nextrun],
                    "<a href=\"$BASESCRIPT?action=misc&operation=cron&edit=$cron[cronid]\" class=\"act\">$lang[edit]</a><br />" . ($cron['available'] ? " <a href=\"$BASESCRIPT?action=misc&operation=cron&run=$cron[cronid]\" class=\"act\">$lang[misc_cron_run]</a>" : " <a href=\"###\" class=\"act\" disabled>$lang[misc_cron_run]</a>")
                ));
            }
        ...
        $cron['filename'] = str_replace(array(
            '..',
            '/',
            '\\'
        ), array(
    ...
    if (! submitcheck('tagsearchsubmit', 1)) {
        ...
        showsetting('misc_tag_threadnum_between', array(
            'threadnumhigher',
            'threadnumlower'
        ), array(), 'range');
        showsetting('misc_tag_status', array(
            'status',
            array(
                array(
                    - 1,
                ...
                array(
                    1,
                    lang('misc_tag_status_1') . "($tagcount[1])"
                ),
                array(
                    0,
                    lang('misc_tag_status_0') . "($tagcount[2])"
                )
            ),
            TRUE
        ), - 1, 'mradio');
        ...
        $sqladd = $tagname ? "tagname LIKE '%" . str_replace(array(
            '%',
            '*',
            '_'
        ), array(
    ...
    if (! $do) {
        ...
        if (! submitcheck('optionsubmit')) {
            ...
            $multipage = $inajax ? multi($num, $mpp, $page, $BASESCRIPT . '?action=misc&operation=custommenu', 0, 3, TRUE, TRUE) : multi($num, $mpp, $page, $BASESCRIPT . '?action=misc&operation=custommenu');
    ...
    if (! $do) {
        ...
        if (! submitcheck('submit')) {
            ...
            showsubmenu('settings_styles', array(
                ...
                array(
                    'nav_settings_forumdisplay',
                    'settings&operation=styles&anchor=forumdisplay',
                    0
                ),
                array(
                    'nav_settings_viewthread',
                ...
                array(
                    'nav_settings_refresh',
                    'settings&operation=styles&anchor=refresh',
                    0
                ),
                array(
                    'nav_settings_sitemessage',
...
EOT;
        ...
        if (! submitcheck('editsubmit')) {
            ...
            showsetting('misc_customnav_style', array(
                'stylenew',
                array(
                    lang('misc_customnav_style_underline'),
                    lang('misc_customnav_style_italic'),
                    lang('misc_customnav_style_bold')
                )
            ), $string[0], 'binmcheckbox');
            showsetting('misc_customnav_style_color', array(
                'colornew',
                array(
                    array(
                        0,
                        '<span style="color: ' . LINK . ';">Default</span>'
                    ),
                    array(
                        1,
            ...
            showsetting('misc_customnav_url_open', array(
                'targetnew',
                array(
                    array(
                        0,
                        lang('misc_customnav_url_open_default')
                    ),
                    array(
                        1,
                        lang('misc_customnav_url_open_blank')
                    )
                ),
                TRUE
            ), $nav['target'], 'mradio');
            showsetting('plugins_edit_modules_adminid', array(
                'levelnew',
                array(
                    array(
                        0,
                    ...
                    array(
                        2,
                        lang('usergroups_system_3')
                    ),
                    array(
                        3,
                        lang('usergroups_system_1')
                    )
                )
            ), $nav['level'], 'select');
            showsubmit('editsubmit');
    ...
    if (! $do) {
        ...
        if (! submitcheck('focussubmit')) {
            ...
            showsubmenu('misc_focus', array(
                array(
                    'config',
                    'misc&operation=focus&do=config',
                    0
                ),
                array(
                    'admin',
                    'misc&operation=focus',
                    1
                ),
                array(
                    array(
                        ...
                        'submenu' => array(
                            array(
                                'misc_focus_handadd',
                                'misc&operation=focus&do=handadd'
                            ),
                            array(
                                'misc_focus_threadadd',
                                'misc&operation=focus&do=threadadd'
                            ),
                            array(
                                'misc_focus_autoadd',
                                'misc&operation=focus&do=recommend'
                            )
                        )
                    ),
                    '',
            ...
            $newfocus['data'] = array();
            if (isset($focus['data']) && is_array($focus['data']))
                foreach ($focus['data'] as $k => $v) {
...
EOT;
        ...
        if (! submitcheck('editsubmit')) {
            ...
            if ($item['type'] == 'thread') {
                ...
                if ($attachkeys) {
                    showsetting('misc_focus_image', array(
                        'focus_aid',
                        $attachlist
                    ), $item['aid'], 'select');
