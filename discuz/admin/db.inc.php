...
$excepttables = array(
    $tablepre . 'adminsessions',
    $tablepre . 'failedlogins',
    $tablepre . 'pmsearchindex',
    $tablepre . 'relatedthreads',
    $tablepre . 'rsscaches',
    $tablepre . 'searchindex',
    $tablepre . 'spacecaches',
    $tablepre . 'sessions'
);
...
if ($operation == 'export') {
    ...
    if (! submitcheck('exportsubmit', 1)) {
        ...
        showsubmenu('nav_db', array(
            ...
            array(
                'nav_db_runquery',
                'db&operation=runquery',
                0
            ),
            array(
                'nav_db_optimize',
                'db&operation=optimize',
                0
            ),
            array(
                'nav_db_dbcheck',
                'db&operation=dbcheck',
                0
            )
        ...
        showtableheader();
        showsetting('db_export_type', array(
            'type',
            array(
                array(
                    'discuz',
                    $lang['db_export_discuz'],
                    array(
                        'showtables' => 'none'
                    )
                ),
        ...
        showsetting('db_export_options_sql_compatible', array(
            'sqlcompat',
            array(
                array(
                    '',
        ...
        showsetting('db_export_options_charset', array(
            'sqlcharset',
            array(
                array(
                    '',
                ...
                $dbcharset ? array(
                    $dbcharset,
                    strtoupper($dbcharset)
                ) : array(),
                $db->version() > '4.1' && $dbcharset != 'utf8' ? array(
                    'utf8',
                    'UTF-8'
                ) : array()
            ),
        ...
        if (function_exists('gzcompress')) {
            showsetting('db_export_usezip', array(
                'usezip',
                array(
                    array(
                        '1',
                        $lang['db_export_zip_1']
                    ),
                    array(
                        '2',
                        $lang['db_export_zip_2']
                    ),
                    array(
                        '0',
                        $lang['db_export_zip_3']
                    )
                )
            ), 0, 'mradio');
        }
        ...
        $backupfilename = './forumdata/' . $backupdir . '/' . str_replace(array(
            '/',
            '\\',
            '.'
        ), '', $filename);
    ...
    if (! submitcheck('importsubmit', 1) && ! submitcheck('deletesubmit')) {
        ...
        showtablerow('', array(
            'colspan="2" class="rowform"',
            'colspan="7" class="rowform"'
        ), array(
        ...
        if ($dumpinfo['method'] == 'multivol') {
            ...
            if ($dumpinfo['volume'] == 1) {
                cpmsg('database_import_multivol_prompt', "$BASESCRIPT?action=db&operation=import&from=server&datafile_server=$datafile_next&autoimport=yes&importsubmit=yes" . (! empty($delunzip) ? '&delunzip=yes' : ''), 'form');
            } elseif ($autoimport) {
        ...
        if (is_array($delete)) {
            foreach ($delete as $filename) {
                $file_path = './forumdata/' . $backupdir . '/' . str_replace(array(
                    '/',
                    '\\'
                ), '', $filename);
    ...
    if (! submitcheck('sqlsubmit')) {
        ...
        $sqlquery = splitsql(str_replace(array(
            ' cdb_',
            ' {tablepre}',
            ' `cdb_'
        ), array(
        ...
        if (submitcheck('repairsubmit') && (! empty($repair) || ! empty($setting) || ! empty($repairtable) || ! empty($missingtable))) {
            ...
            foreach ($repair as $value) {
                if (! in_array($r_table, $repairtable)) {
                    ...
                    $altersql .= in_array($fielddefault[$r_table][$field['Field']], array(
                        '',
                        '0'
                    )) && in_array($field['Default'], array(
                        '',
                        '0'
                    )) || $field['Null'] == 'NO' && $field['Default'] == '' || preg_match('/text/i', $field['Type']) || preg_match('/auto_increment/i', $field['Extra']) ? '' : ' default \'' . $field['Default'] . '\'';
            ...
            foreach ($repairtable as $value) {
                foreach ($discuzdb[$value] as $field) {
                    ...
                    $altersql = '`' . $field['Field'] . '` ' . $field['Type'];
                    $altersql .= $field['Null'] == 'NO' ? ' NOT NULL' : '';
                    $altersql .= in_array($fielddefault[$value][$field['Field']], array(
                        '',
                        '0'
                    )) && in_array($field['Default'], array(
                        '',
        ...
        foreach ($discuzdb as $dbtable => $fields) {
            ...
            if ($fields != $discuzdbnew[$dbtable]) {
                foreach ($discuzdb[$dbtable] as $key => $value) {
                    if (is_array($missingtables) && in_array($tablepre . $dbtable, $missingtables)) {} elseif (! isset($discuzdbnew[$dbtable][$key])) {
                        $dellist[] = $value;
            ...
            if (($modifylist || $dellist) && ! in_array($dbtable, $excepttables)) {
                ...
                $showlist .= showtablerow('', '', array(
                    "<span class=\"diffcolor3\">$tablepre$dbtable</span> $lang[dbcheck_field]",
                    $lang[dbcheck_org_field],
                    $lang[dbcheck_status]
                ), TRUE);
                ...
                foreach ($modifylist as $value) {
                    ...
                    $showlist .= "<tr><td><input name=\"repair[]\" class=\"checkbox\" type=\"checkbox\" value=\"$dbtable|$value[Field]|modify\"> <b>" . $value['Field'] . "</b> " . $discuzdbnew[$dbtable][$value['Field']]['Type'] . ($discuzdbnew[$dbtable][$value['Field']]['Null'] == 'NO' ? ' NOT NULL' : '') . (! preg_match('/auto_increment/i', $discuzdbnew[$dbtable][$value['Field']]['Extra']) && ! preg_match('/text/i', $discuzdbnew[$dbtable][$value['Field']]['Type']) ? ' default \'' . $discuzdbnew[$dbtable][$value['Field']]['Default'] . '\'' : '') . ' ' . $discuzdbnew[$dbtable][$value['Field']]['Extra'] . "</td><td><b>" . $value['Field'] . "</b> " . $value['Type'] . ($value['Null'] == 'NO' ? ' NOT NULL' : '') . (! preg_match('/auto_increment/i', $value['Extra']) && ! preg_match('/text/i', $value['Type']) ? ' default \'' . $value['Default'] . '\'' : '') . ' ' . $value['Extra'] . "</td><td>" . (! $slowstatus ? "<em class=\"edited\">$lang[dbcheck_modify]</em></td></tr>" : "<em class=\"unknown\">$lang[dbcheck_slow]</em>") . "</td></tr>";
                }
                ...
                foreach ($dellist as $value) {
                    $showlist .= "<tr><td><input name=\"repair[]\" class=\"checkbox\" type=\"checkbox\" value=\"$dbtable|$value[Field]|add\"> <strike><b>" . $value['Field'] . "</b></strike></td><td> <b>" . $value['Field'] . "</b> " . $value['Type'] . ($value['Null'] == 'NO' ? ' NOT NULL' : '') . "</td><td>" . "<em class=\"del\">$lang[dbcheck_delete]</em></td></tr>";
                }
            }
    ...
    showsubmenusteps('', array(
        array(
            'nav_filecheck_confirm',
            $step == 1
        ),
        array(
            'nav_filecheck_verify',
            $step == 2
        ),
        array(
            'nav_filecheck_completed',
            $step == 3
        )
    ));
...
function syntablestruct($sql, $version, $dbcharset)
{
    ...
    if ($sqlversion === $version) {
        ...
        return $sqlversion && $dbcharset ? preg_replace(array(
            '/ character set \w+/i',
            '/ collate \w+/i',
