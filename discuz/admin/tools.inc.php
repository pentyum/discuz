...
if ($operation == 'updatecache') {
    ...
    showsubmenusteps('nav_updatecache', array(
        array(
            'nav_updatecache_confirm',
            $step == 1
        ),
        array(
            'nav_updatecache_verify',
            $step == 2
        ),
        array(
            'nav_updatecache_completed',
            $step == 3
        )
    ));
    ...
    if (! submitcheck('submit')) {
        ...
        foreach ($applist as $data) {
            $appid = $data['appid'];
            $status = array(
                intval($relatedtag['status'][$appid]) => 'checked="checked"'
            );
            ...
            echo '<tr><td class="td27" colspan="2">' . $lang['extended_tag_tpl'] . '</td></tr><tr><td class="vtop rowform">' . '<textarea cols="100" rows="8" id="jstemplate_' . $appid . '" name="relatedtagnew[template][' . $appid . '][template]" class="tarea">' . $template . '</textarea>' . '<input type="hidden" name="relatedtagnew[template][' . $appid . '][name]" value="' . $data['name'] . '" /></td><td class="vtop"><div class="extcredits">';
            ...
            showhiddenfields(array(
                'relatedtagnew[template][' . $appid . '][name]' => $data['name']
            ));
        }
    ...
    showsubmenusteps('nav_fileperms', array(
        array(
            'nav_fileperms_confirm',
            $step == 1
        ),
        array(
            'nav_fileperms_verify',
            $step == 2
        ),
        array(
            'nav_fileperms_completed',
            $step == 3
        )
    ));
