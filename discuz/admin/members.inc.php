...
if (! $operation) {
    ...
    if (! submitcheck('submit', 1)) {
        ...
        showsubmenu('nav_members', array(
            ...
            array(
                'clean',
                'members&operation=clean',
                0
            ),
            array(
                'nav_repeat',
                'members&operation=repeat',
                0
            )
        ));
        ...
        while ($member = $db->fetch_array($query)) {
            ...
            $members .= showtablerow('', array(
                ...
                "<a href=\"$BASESCRIPT?action=members&operation=group&uid=$member[uid]\" class=\"act\">$lang[usergroup]</a><a href=\"$BASESCRIPT?action=members&operation=access&uid=$member[uid]\" class=\"act\">$lang[members_access]</a>" . ($extcredits ? "<a href=\"$BASESCRIPT?action=members&operation=credit&uid=$member[uid]\" class=\"act\">$lang[credits]</a>" : "<span disabled>$lang[edit]</span>") . "<a href=\"$BASESCRIPT?action=members&operation=medal&uid=$member[uid]\" class=\"act\">$lang[medals]</a>" . "<a href=\"$BASESCRIPT?action=members&operation=repeat&uid=$member[uid]\" class=\"act\">$lang[members_repeat]</a>" . "<a href=\"$BASESCRIPT?action=members&operation=edit&uid=$member[uid]\" class=\"act\">$lang[detail]</a>"
            ), TRUE);
        }
        ...
        showtablerow('', array(
            'class="td25"',
            'class="lineheight" colspan="7"'
        ), array(
            '',
            lang('members_admin_comment')
        ));
    ...
    if (! submitcheck('newslettersubmit', 1)) {
        ...
        if (submitcheck('submit', 1)) {
            ...
            if (! $membernum) {
                ...
                eval("\$lang[members_search_result] = \"" . $lang['members_search_result'] . "\";");
                showtablerow('class="first"', array(
                    'class="th11"'
                ), array(
    ...
    if (! submitcheck('rewardsubmit', 1)) {
        ...
        if (submitcheck('submit', 1)) {
            ...
            if (! $membernum) {
                ...
                $creditsvalue = array_merge(array(
                    '<input type="radio" name="updatecredittype" id="updatecredittype0" value="0" class="radio" onclick="var extcredits = new Array(' . $js_extcreditids . '); for(k in extcredits) {$(\'resetextcredits[\'+extcredits[k]+\']\').disabled = true; $(\'addextcredits[\'+extcredits[k]+\']\').disabled = false;}" checked="checked" /><label for="updatecredittype0">' . $lang['members_reward_value'] . '</label>'
                ), $creditsvalue);
                $resetcredits = array_merge(array(
                    '<input type="radio" name="updatecredittype" id="updatecredittype1" value="1" class="radio" onclick="var extcredits = new Array(' . $js_extcreditids . '); for(k in extcredits) {$(\'addextcredits[\'+extcredits[k]+\']\').disabled = true; $(\'resetextcredits[\'+extcredits[k]+\']\').disabled = false;}" /><label for="updatecredittype1">' . $lang['members_reward_clean'] . '</label>'
                ), $resetcredits);
    ...
    if (! submitcheck('addsubmit')) {
        ...
        $newadminid = in_array($group['radminid'], array(
            1,
            2,
            3
        )) ? $group['radminid'] : ($group['type'] == 'special' ? - 1 : 0);
        if ($group['radminid'] == 1) {
            cpmsg('members_add_admin_none', '', 'error');
        }
        if (in_array($group['groupid'], array(
            5,
            6,
            7
        ))) {
    ...
    if (! submitcheck('editsubmit')) {
        ...
        $checkadminid = array(
            ($member['adminid'] >= 0 ? $member['adminid'] : 0) => 'checked'
        );
        ...
        if ($member['groupterms']['main']) {
            ...
            $selecteaid = array(
                $member['groupterms']['main']['adminid'] => 'selected'
            );
            $selectegid = array(
                $member['groupterms']['main']['groupid'] => 'selected'
            );
        } else {
            $expirydays = '';
            $expirydate = $member['groupexpiry'] ? gmdate('Y-n-j', $member['groupexpiry'] + $timeoffset * 3600) : '';
            $selecteaid = array(
                $member['adminid'] => 'selected'
            );
            $selectegid = array(
                ($member['grouptype'] == 'member' ? 0 : $member['groupid']) => 'selected'
            );
        }
        ...
        $group = array(
            ...
            'radminid' => 0,
            'type' => '',
            'grouptitle' => $lang['usergroups_system_0'],
            'creditshigher' => 0,
            'creditslower' => '0'
        );
        ...
        do {
            if ($group['groupid'] && ! in_array($group['groupid'], array(
                4,
                5,
                6,
                7,
                8
            )) && ($group['type'] == 'system' || $group['type'] == 'special')) {
                $extgroups .= showtablerow('', array(
                    'class="td27"',
                    'style="width:70%"'
                ), array(
        ...
        switch ($group['type']) {
            case 'member':
                $groupidnew = in_array($adminidnew, array(
                    1,
                    2,
                    3
                )) ? $adminidnew : $groupidnew;
                break;
            case 'special':
                if ($group['radminid']) {
                    $adminidnew = $group['radminid'];
                } elseif (! in_array($adminidnew, array(
                    1,
                    2,
                    3
                ))) {
                ...
                break;
            case 'system':
                $adminidnew = in_array($groupidnew, array(
                    1,
                    2,
                    3
                )) ? $groupidnew : - 1;
        ...
        if ($expirydatenew) {
            ...
            if (! $group) {
                $expgroupidnew = in_array($expadminidnew, array(
                    1,
                    2,
                    3
                )) ? $expadminidnew : $expgroupidnew;
            } else {
                switch ($group['type']) {
                    case 'special':
                        if ($group['radminid']) {
                            $expadminidnew = $group['radminid'];
                        } elseif (! in_array($expadminidnew, array(
                            1,
                            2,
                            3
                        ))) {
                        ...
                        break;
                    case 'system':
                        $expadminidnew = in_array($expgroupidnew, array(
                            1,
                            2,
                            3
                        )) ? $expgroupidnew : - 1;
            ...
            if ($expgroupidnew == $groupidnew) {
                ...
                if ($expgroupidnew || $expadminidnew) {
                    $groupterms['main'] = array(
                        'time' => $maingroupexpirynew,
                        'adminid' => $expadminidnew,
                        'groupid' => $expgroupidnew
                    );
                } else {
                    $groupterms['main'] = array(
                        'time' => $maingroupexpirynew
        ...
        if ($groupidnew != $member['groupid'] && (in_array($groupidnew, array(
            4,
            5
        )) || in_array($member['groupid'], array(
            4,
            5
        )))) {
    ...
    if (! submitcheck('creditsubmit')) {
        ...
        if ($jscreditsformula = $db->result_first("SELECT value FROM {$tablepre}settings WHERE variable='creditsformula'")) {
            $jscreditsformula = str_replace(array(
                'digestposts',
                'posts',
                'threads',
                'pageviews',
        ...
        $creditsvalue = array(
            $member['type'] == 'member' ? "$member[creditshigher]~$member[creditslower]" : 'N/A',
            '<input type="text" class="txt" name="jscredits" id="jscredits" value="' . $membercredit . '" size="3" disabled>'
        );
        for ($i = 1; $i <= 8; $i ++) {
...
EOT;
    ...
    if (! empty($username) || ! empty($uid)) {
        ...
        if (! $member) {
            cpmsg('members_edit_nonexistence', '', 'error');
        } elseif (($member['grouptype'] == 'system' && in_array($member['groupid'], array(
            1,
            2,
            3,
            6,
    ...
    if (! submitcheck('bansubmit')) {
        ...
        showsetting('members_ban_type', array(
            'bannew',
            array(
                array(
                    '',
                ...
                array(
                    'post',
                    $lang['members_ban_post']
                ),
                array(
                    'visit',
                    $lang['members_ban_visit']
                )
            )
        ), '0', 'mradio');
        showsetting('members_ban_validity', '', '', selectday('banexpirynew', array(
            0,
            1,
            3,
            5,
    ...
    if (! submitcheck('accesssubmit')) {
        ...
        showsubtitle(array(
            ...
            'members_access_view',
            'members_access_post',
            'members_access_reply',
            'members_access_postattach',
            'members_access_getattach',
            'members_access_adminuser',
            'members_access_dateline'
        ));
        ...
        foreach (array(
            ...
            showsetting('members_access_add_' . $perm, array(
                'allow' . $perm . 'new',
                array(
                    array(
                        0,
                    ...
                    array(
                        1,
                        lang('members_access_allowed')
                    ),
                    array(
                        - 1,
                        lang('members_access_disallowed')
                    )
                ),
                TRUE
            ), 0, 'mradio');
    ...
    if (! submitcheck('editsubmit')) {
        ...
        $tfcheck = array(
            $member['timeformat'] => 'checked'
        );
        $gendercheck = array(
            $member['gender'] => 'checked'
        );
        $pscheck = array(
            $member['pmsound'] => 'checked'
        );
    ...
    if (! submitcheck('ipbansubmit')) {
        ...
        showtablerow('', array(
            '',
            'class="td28" colspan="3"',
            'class="td28" colspan="2"'
        ), array(
            $lang['add_new'],
...
function searchmembers($operation = '')
{
    ...
    showsetting("$lang[credits] $lang[members_search_between]", array(
        "higher[credits]",
        "lower[credits]"
    ), array(
        $higher[credits],
    ...
    if (! empty($extcredits)) {
        foreach ($extcredits as $id => $credit) {
            showsetting("$credit[title] $lang[members_search_between]", array(
                "higher[extcredits$id]",
                "lower[extcredits$id]"
            ), array(
                $higher['extcredits' . $id],
                $lower['extcredits' . $id]
    ...
    showsetting('members_search_postsrange', array(
        'postshigher',
        'postslower'
    ), array(
        $postshigher,
    ...
    showsetting('members_search_regdaterange', array(
        'regdateafter',
        'regdatebefore'
    ), array(
        $regdateafter,
    ...
    showsetting('members_search_lastvisitrange', array(
        'lastvisitafter',
        'lastvisitbefore'
    ), array(
        $lastvisitafter,
    ...
    showsetting('members_search_lastpostrange', array(
        'lastpostafter',
        'lastpostbefore'
    ), array(
        $lastpostafter,
...
function countmembers()
{
    ...
    if ($username != '') {
        ...
        foreach ($usernames as $u) {
            $usernameary[] = " username LIKE '" . str_replace(array(
                '%',
                '*',
                '_'
            ), array(
...
function notifymembers($operation, $variable)
{
    ...
    if (! empty($current)) {
        ...
        $db->query("REPLACE INTO {$tablepre}settings (variable, value) VALUES ('$variable', '" . addslashes(serialize(array(
            'subject' => $subject,
            'message' => $message
        ))) . "')");
    }
...
{
    return $access == - 1 ? '<img src="images/common/access_disallow.gif" />' : ($access == 1 ? '<img src="images/common/access_allow.gif" />' : '<img src="images/common/access_normal.gif" />');
}
