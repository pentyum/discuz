...
if (! isset($myapps)) {
    ...
    writetocache('manyou', '', getcachevars(array(
        'myapps' => $myapps
    )));
}
...
$view = ! empty($view) && in_array($view, array(
    'me',
    'all',
    'friend'
)) ? $view : 'friend';
...
foreach ($feeds['data'] as $k => $feed) {
    $trans['{addbuddy}'] = $view == 'all' && $feed['uid'] != $discuz_uid ? '<a href="my.php?item=buddylist&newbuddyid=' . $feed['uid'] . '&buddysubmit=yes" id="ajax_buddy_' . ($bi ++) . '" title="���Ϊ����" onclick="ajaxmenu(this, 3000);doane(event);"><img style="vertical-align:middle" src="manyou/images/myadd.gif" /></a>' : '';
