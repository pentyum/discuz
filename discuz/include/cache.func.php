...
function updatecache($cachename = '')
{
    ...
    if ($maxbdays) {
        $cachescript['birthdays'] = array(
            'birthdays'
        );
        $cachescript['index'][] = 'birthdays_index';
    }
    
    $updatelist = empty($cachename) ? array_values($cachescript) : (is_array($cachename) ? array(
        '0' => $cachename
    ) : array(
        array(
            '0' => $cachename
    ...
    if (! $cachename || $cachename == 'threadsorts') {
        ...
        while ($data = $db->fetch_array($query)) {
            ...
            } elseif (in_array($data['type'], array(
                'text',
                'textarea'
            ))) {
...
function getcachearray($cachename, $script = '')
{
    ...
    $data = array();
    if (! in_array($cachename, array(
        'focus',
        'secqaa',
        'heats'
    )) && substr($cachename, 0, 5) != 'advs_') {
    ...
    switch ($cachename) {
        case 'settings':
            while ($setting = $db->fetch_array($query)) {
                ...
                } elseif (in_array($setting['variable'], array(
                    'creditspolicy',
                    'ftp',
                    'secqaa',
                    'ec_credit',
            ...
            while ($prompttype = $db->fetch_array($query)) {
                ...
                $data['prompts'][$prompttype['key']] = array(
                    'name' => $prompttype['name'],
                    'script' => $prompttype['script'],
                    'id' => $prompttype['id'],
                    'new' => 0
                );
            ...
            while ($plugin = $db->fetch_array($query)) {
                    ...
                    foreach ($plugin['modules'] as $module) {
                        if ($plugin['available'] && isset($module['name'])) {
                            ...
                            switch ($module['type']) {
                                ...
                                case 9:
                                    $k = ! $k ? 'plinks_tools' : $k;
                                    $data['plugins'][$k][] = array(
                                        'displayorder' => $module['displayorder'],
                                        'adminid' => $module['adminid'],
                                        'url' => "<a id=\"mn_plink_$module[name]\" href=\"$module[url]\">$module[menu]</a>"
                                    );
                                    break;
                                case 2:
                                    $k = 'links';
                                ...
                                case 10:
                                    ...
                                    $data['plugins'][$k][] = array(
                                        'displayorder' => $module['displayorder'],
                                        'id' => $plugin['identifier'] . ':' . $module['name'],
                                        'adminid' => $module['adminid'],
                                        'url' => "<a id=\"mn_plugin_$plugin[identifier]_$module[name]\" href=\"plugin.php?id=$plugin[identifier]:$module[name]\">$module[menu]</a>"
                                    );
                                    $data['pluginlinks'][$plugin['identifier']][$module['name']] = array(
                                        'adminid' => $module['adminid'],
                                        'directory' => $plugin['directory']
                                    );
                                    break;
                                case 13:
                                    $data['plugins']['script'][$plugin['identifier']][$module['name']] = array(
                                        'directory' => $plugin['directory']
                                    );
                                    break;
                                case 14:
                                ...
                                case 16:
                                    $k = ! $k ? 'modcp_tools' : $k;
                                    $data['plugins'][$k][$plugin['identifier'] . ':' . $module['name']] = array(
                                        'displayorder' => $module['displayorder'],
                                        'name' => $module['menu'],
                                        'directory' => $plugin['directory']
                                    );
                                ...
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    $data['plugins']['include'][$plugin['identifier']] = array(
                                        'displayorder' => $module['displayorder'],
                                        'adminid' => $module['adminid'],
                                    ...
                                    if (class_exists('plugin_' . $plugin['identifier'])) {
                                        ...
                                        foreach ($hookmethods as $funcname) {
                                            ...
                                            if (preg_match('/\_output$/', $funcname)) {
                                                ...
                                                $data['hookscript'][$curscript]['outputfuncs'][$varname][] = array(
                                                    'displayorder' => $module['displayorder'],
                                                    'func' => array(
                                                        $plugin['identifier'],
                                                        $funcname
                                                ...
                                                $data['hookscript'][$curscript]['funcs'][$funcname][] = array(
                                                    'displayorder' => $module['displayorder'],
                                                    'func' => array(
                                                        $plugin['identifier'],
                                                        $funcname
                ...
                if ($addadminmenu) {
                    $adminmenu[] = array(
                        'url' => "plugins&operation=config&pluginid=$plugin[pluginid]",
                        'name' => $plugin['name']
                    );
                }
            }
            $data['my_status'] = $data['plugins']['available'] && in_array('manyou', $data['plugins']['available']) ? $data['my_status'] : 0;
            writetocache('scriptlang', '', getcachevars(array(
            ...
            writetocache('adminmenu', '', getcachevars(array(
                'adminmenu' => $adminmenu
            )), '');
        ...
        case 'custominfo':
            ...
            if (! empty($data['extcredits'])) {
                foreach ($data['extcredits'] as $key => $value) {
                    if ($value['available']) {
                        ...
                        $authorinfoitems['extcredits' . $key] = array(
                            $value['title'],
                            '$post[extcredits' . $key . '] {$extcredits[' . $key . '][unit]}'
                        );
                    }
                }
            }
            ...
            while ($field = $db->fetch_array($query)) {
                ...
                if ($field['selective']) {
                    ...
                    $authorinfoitems['field_' . $field['fieldid']] = array(
                        $field['title'],
                        '{$profilefields[' . $field['fieldid'] . '][$post[field_' . $field['fieldid'] . ']]}'
                    );
                } else {
                    $authorinfoitems['field_' . $field['fieldid']] = array(
                        $field['title'],
        ...
        case 'forums':
            ...
            while ($forum = $db->fetch_array($query)) {
                foreach ($perms as $perm) {
                    ...
                    foreach ($forumnoperm as $groupid) {
                        ...
                        $forumnoperms[$forum['fid']][$perm][$groupid] = array(
                            $nopermtype,
                            $permgroups
                        );
                    }
            ...
            writetocache('nopermission', '', getcachevars(array(
                'noperms' => $forumnoperms
            )));
            break;
        case 'onlinelist':
            $data['legend'] = '';
    ...
    $cachename = in_array(substr($cachename, 0, 5), array(
        'advs_',
        'tags_'
    )) ? substr($cachename, 0, 4) : $cachename;
    ...
    if ($db->num_rows($query)) {
        while ($adv = $db->fetch_array($query)) {
            if (in_array($adv['type'], array(
                ...
                $position = isset($parameters['position']) && in_array($parameters['position'], array(
                    2,
                    3
            ...
            $adv['targets'] = in_array($adv['targets'], array(
                '',
                'all'
            )) ? ($type == 'text' ? 'forum' : (substr($type, 0, 6) == 'thread' ? 'forum' : 'all')) : $adv['targets'];
            foreach (explode("\t", $adv['targets']) as $target) {
                ...
                $target = $target == '0' || $type == 'intercat' ? 'index' : (in_array($target, array(
                    'all',
                    'index',
                    'forumdisplay',
                    'viewthread',
                    ...
                    'redirect',
                    'archiver'
                )) ? $target : ($target == 'forum' ? 'forum_all' : 'forum_' . $target));
                if ((($range == 'forumdisplay' && ! in_array($adv['type'], array(
                    'thread',
                    'interthread'
                ))) || $range == 'viewthread') && substr($target, 0, 6) == 'forum_') {
                    if ($adv['type'] == 'thread') {
                        foreach (isset($parameters['displayorder']) ? explode("\t", $parameters['displayorder']) : array(
                            '0'
                        ) as $postcount) {
                    ...
                    $advs['items'][$adv['advid']] = $adv['code'];
                } elseif ($range == 'all' && in_array($target, array(
                    'all',
                    'redirect'
                ))) {
                    ...
                    $parameters = unserialize($adv['parameters']);
                    foreach (is_array($parameters['position']) ? $parameters['position'] : array(
                        '0'
                    ) as $position) {
