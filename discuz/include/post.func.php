    ...
    foreach ($attachments as $key => $attach) {
        ...
        if ($attach_saved) {
            ...
            if ($attach['isimage'] || $attach['ext'] == 'swf') {
                ...
                $size = $width * $height;
                if ($size > 16777216 || $size < 4 || empty($type) || ($attach['isimage'] && ! in_array($type, array(
                    1,
                    2,
                    3,
                    6,
...
function updatepostcredits($operator, $uidarray, $creditsarray)
{
    ...
    foreach ((is_array($uidarray) ? $uidarray : array(
        $uidarray
    )) as $id) {
    ...
    if ($id) {
        ...
        foreach ($_DTYPE as $optionid => $option) {
            ...
            if ($_DTYPE[$optionid]['type'] == 'radio') {
                $optionlist[$optionid]['value'] = array(
                    $optiondata[$optionid] => 'checked="checked"'
                );
            } elseif ($_DTYPE[$optionid]['type'] == 'select') {
                $optionlist[$optionid]['value'] = array(
                    $optiondata[$optionid] => 'selected="selected"'
                );
            } elseif ($_DTYPE[$optionid]['type'] == 'checkbox') {
                foreach (explode("\t", $optiondata[$optionid]) as $value) {
                    $optionlist[$optionid]['value'][$value] = array(
...
function iswhitelist($host)
{
    ...
    if (is_array($_DCACHE['domainwhitelist']))
        foreach ($_DCACHE['domainwhitelist'] as $val) {
