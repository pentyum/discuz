...
} elseif ((! $forum['ismoderator'] || ! $alloweditpost || (in_array($orig['adminid'], array(
    1,
    2,
    3
)) && $adminid > $orig['adminid'])) && ! ($forum['alloweditpost'] && $isorigauthor)) {
...
if (! submitcheck('editsubmit')) {
    ...
    if (empty($delete)) {
        ...
        if ($isfirstpost) {
            ...
            $price = intval($price);
            $price = $thread['price'] < 0 && ! $thread['special'] ? ($isorigauthor || ! $price ? - 1 : $price) : ($maxprice ? ($price <= $maxprice ? ($price > 0 ? $price : 0) : $maxprice) : ($isorigauthor ? $price : $thread['price']));
            ...
            if ($forum['threadsorts']['types'][$sortid] && $optiondata && is_array($optiondata)) {
                ...
                foreach ($optiondata as $optionid => $value) {
                    ...
                    if (($_DTYPE[$optionid]['search'] || in_array($_DTYPE[$optionid]['type'], array(
                        'radio',
                        'select',
                        'number'
                    ))) && $value) {
        ...
        if ($isfirstpost) {
            ...
            if ($globalstick && in_array($thread['displayorder'], array(
                2,
                3
            ))) {
