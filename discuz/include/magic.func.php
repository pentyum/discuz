...
function getpostinfo($id, $type, $colsarray = '')
{
    ...
    switch ($type) {
        case 'tid':
            $sql = "SELECT $cols FROM {$tablepre}threads WHERE tid='$id' AND digest>='0' AND displayorder>='0'";
            break;
        case 'pid':
            $sql = "SELECT $cols FROM {$tablepre}posts p, {$tablepre}threads t WHERE pid='$id' AND invisible='0' AND t.tid=p.tid AND digest>=0";
            break;
    }
...
function magicthreadmod($tid)
{
    ...
    while ($threadmod = $db->fetch_array($query)) {
        if (! $threadmod['magicid'] && in_array($threadmod['action'], array(
            'CLS',
            'ECL',
            'STK',
            'EST',
