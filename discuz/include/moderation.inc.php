...
if ($operations && $operations != array_intersect($operations, array(
    'delete',
    'highlight',
    'open',
    'close',
    ...
    'type',
    'move'
)) || (! $allowdelpost && in_array('delete', $operations)) || (! $allowstickthread && in_array('stick', $operations))) {
        ...
        foreach ($operations as $operation) {
            ...
            if ($operation == 'stick') {
                ...
                foreach ($threadlist as $thread) {
                    $stickmodify = (in_array($thread['displayorder'], array(
                        2,
                        3
                    )) || in_array($sticklevel, array(
                        2,
                        3
                    ))) && $sticklevel != $thread['displayorder'] ? 1 : $stickmodify;
                }
...
function checkexpiration($expiration)
{
    ...
    if (! empty($expiration) && in_array($operation, array(
        'recommend',
        'stick',
        'digest',
        'highlight',
