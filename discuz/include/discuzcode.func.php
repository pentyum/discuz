...
function censor($message)
{
    ...
    if ($_DCACHE['censor']['banned']) {
        ...
        if (preg_match($_DCACHE['censor']['banned'], @preg_replace(array(
            "/\[($bbcodes)=?.*\]/iU",
            "/\[\/($bbcodes)\]/i"
        ), '', $message))) {
...
function discuzcode($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0)
{
    ...
    return $htmlon ? $message : nl2br(str_replace(array(
        "\t",
        '   ',
        '  '
    ), array(
...
function parsetable($width, $bgcolor, $message)
{
    ...
    return '<table cellspacing="0" class="t_table" ' . ($width == '' ? NULL : 'style="width:' . $width . '"') . ($bgcolor ? ' bgcolor="' . $bgcolor . '">' : '>') . str_replace('\\"', '"', preg_replace(array(
...
function bbcodeurl($url, $tags)
{
    if (! preg_match("/<.+?>/s", $url)) {
        ...
        return str_replace(array(
            ...
            '',
            ''
        ), sprintf($tags, $url, addslashes($url)));
    } else {
        return '&nbsp;' . $url;
    }
}
