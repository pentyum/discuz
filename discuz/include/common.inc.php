...
if ($gzipcompress && function_exists('ob_gzhandler') && ! in_array(CURSCRIPT, array(
    'attachment',
    'wap'
)) && ! $inajax) {
...
list ($discuz_pw, $discuz_secques, $discuz_uid) = empty($_DCOOKIE['auth']) ? array(
    '',
    '',
    0
) : daddslashes(explode("\t", authcode($_DCOOKIE['auth'], 'DECODE')), 1);
...
$timenow = array(
    'time' => gmdate("$dateformat $timeformat", $timestamp + 3600 * $timeoffset),
...
if (empty($discuz_uid) || empty($discuz_user)) {
    ...
    $discuz_uid = $adminid = $posts = $digestposts = $pageviews = $oltime = $invisible = $credits = $extcredits1 = $extcredits2 = $extcredits3 = $extcredits4 = $extcredits5 = $extcredits6 = $extcredits7 = $extcredits8 = 0;
...
if ($discuz_uid && $_DSESSION) {
    if (! empty($groupexpiry) && $groupexpiry < $timestamp && ! in_array(CURSCRIPT, array(
        'wap',
        'member'
    ))) {
...
} elseif (! (in_array(CURSCRIPT, array(
    'logging',
    'wap',
    'seccode',
    'ajax'
