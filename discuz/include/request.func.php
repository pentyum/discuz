...
function parse_request($requestdata, $cachefile, $mode, $specialfid = 0, $key = '')
{
    ...
    if (! in_array($requestdata['function'], array(
        'custom',
        'side'
    ))) {
        $requesttemplatebody = '';
...
function updaterequest($requestdata, $requesttemplatebody, $requesttemplate, $specialfid, $mode, $key, &$nocache)
{
    ...
    if ($function == 'threads') {
        $orderby = isset($requestdata['orderby']) ? (in_array($requestdata['orderby'], array(
            'lastpost',
            'dateline',
            'replies',
            'views',
            ...
            'weekviews',
            'monthviews'
        )) ? $requestdata['orderby'] : 'lastpost') : 'lastpost';
        ...
        $orderby = isset($requestdata['orderby']) ? (in_array($requestdata['orderby'], array(
            ...
            'threads',
            'posts'
        )) ? $requestdata['orderby'] : 'displayorder') : 'displayorder';
        $datalist = array();
        ...
        $orderby = isset($requestdata['orderby']) ? (in_array($requestdata['orderby'], array(
            ...
            'weekposts',
            'monthposts'
        )) ? $requestdata['orderby'] : 'credits') : 'credits';
        $hours = isset($requestdata['hours']) ? intval($requestdata['hours']) : 0;
        $datalist = array();
        switch ($orderby) {
            case 'credits':
        ...
        $orderby = isset($requestdata['orderby']) ? (in_array($requestdata['orderby'], array(
            ...
            'weekdownloads',
            'monthdownloads'
        )) ? $requestdata['orderby'] : 'dateline') : 'dateline';
