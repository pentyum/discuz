...
function errorlog($type, $message, $halt = 1)
{
    ...
    writelog('errorlog', dhtmlspecialchars("$timestamp\t$type\t$user\t" . str_replace(array(
        "\r",
        "\n"
    ), array(
        ' ',
...
function formulaperm($formula, $type = 0, $wap = FALSE)
{
    ...
    if (! $formulaperm || $type == 2) {
        if (! $permmessage) {
            ...
            $search = array(
                '$memberformula[\'regdate\']',
                '$memberformula[\'regday\']',
                '$memberformula[\'regip\']',
                '$memberformula[\'lastip\']',
                '$memberformula[\'buyercredit\']',
                '$memberformula[\'sellercredit\']',
                '$_DSESSION[\'digestposts\']',
                '$_DSESSION[\'posts\']',
                '$_DSESSION[\'threads\']',
                '$_DSESSION[\'oltime\']',
                '$_DSESSION[\'pageviews\']'
            ...
            $replace = array(
                $language['formulaperm_regdate'],
                $language['formulaperm_regday'],
                $language['formulaperm_regip'],
                $language['formulaperm_lastip'],
                $language['formulaperm_buyercredit'],
                $language['formulaperm_sellercredit'],
                $language['formulaperm_digestposts'],
                $language['formulaperm_posts'],
                $language['formulaperm_threads'],
                $language['formulaperm_oltime'],
                $language['formulaperm_pageviews']
            );
            for ($i = 1; $i <= 8; $i ++) {
            ...
            $replace = array_merge($replace, array(
                '&nbsp;&nbsp;<b>' . $language['formulaperm_and'] . '</b>&nbsp;&nbsp;',
                '&nbsp;&nbsp;<b>' . $language['formulaperm_or'] . '</b>&nbsp;&nbsp;',
                '&ge;',
                '&le;',
                '='
            ));
...
function getgroupid($uid, $group, &$member)
{
    ...
    if (! empty($creditsformula)) {
        ...
        if ($credits != $member['credits']) {
            ...
            $send_feed = false;
            if (is_array($dzfeed_limit['user_credit']))
                foreach ($dzfeed_limit['user_credit'] as $val) {
...
{
    return preg_match("/^[a-zA-Z0-9_-]{5,19}$/", $wechat);
}
...
function multi($num, $perpage, $curpage, $mpurl, $maxpages = 0, $page = 10, $autogoto = TRUE, $simple = FALSE)
{
    ...
    if ($num > $perpage) {
        ...
        $multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="' . $mpurl . 'page=1" class="first"' . $ajaxtarget . '>1 ...</a>' : '') . ($curpage > 1 && ! $simple ? '<a href="' . $mpurl . 'page=' . ($curpage - 1) . '" class="prev"' . $ajaxtarget . '>' . $lang['prev'] . '</a>' : '');
        for ($i = $from; $i <= $to; $i ++) {
            $multipage .= $i == $curpage ? '<strong>' . $i . '</strong>' : '<a href="' . $mpurl . 'page=' . $i . ($ajaxtarget && $i == $pages && $autogoto ? '#' : '') . '"' . $ajaxtarget . '>' . $i . '</a>';
        }
        
        $multipage .= ($to < $pages ? '<a href="' . $mpurl . 'page=' . $pages . '" class="last"' . $ajaxtarget . '>... ' . $realpages . '</a>' : '') . ($curpage < $pages && ! $simple ? '<a href="' . $mpurl . 'page=' . ($curpage + 1) . '" class="next"' . $ajaxtarget . '>' . $lang['next'] . '</a>' : '') . ($showkbd && ! $simple && $pages > $page && ! $ajaxtarget ? '<kbd><input type="text" name="custompage" size="3" onkeydown="if(event.keyCode==13) {window.location=\'' . $mpurl . 'page=\'+this.value; return false;}" /></kbd>' : '');
...
function updatesession()
{
    ...
    global $db, $tablepre, $sessionexists, $sessionupdated, $sid, $onlineip, $discuz_uid, $discuz_user, $timestamp, $lastactivity, $seccode, $pvfrequence, $spageviews, $lastolupdate, $oltimespan, $onlinehold, $groupid, $styleid, $invisible, $discuz_action, $fid, $tid;
...
function hookscriptoutput($tplfile)
{
    ...
    if (isset($hookscript[CURSCRIPT]['outputfuncs'])) {
        hookscript(CURSCRIPT, 'outputfuncs', array(
            'template' => $tplfile,
            'message' => $hookscriptmessage
        ));
    }
}
...
{
    if (! $GLOBALS['my_status'] || ! in_array($type, array(
        'user',
        'friend'
...
function get_feed($conf = array())
{
    ...
    if ($nocache) {
        ...
        while ($row = $db->fetch_array($query)) {
            ...
            $feed['image'] = array();
            if (is_array($row['data']['image']))
                foreach ($row['data']['image'] as $val) {
