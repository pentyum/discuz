...
if (! $resultempty) {
    ...
    while ($myrepeat = $db->fetch_array($query)) {
        ...
        $i ++;
        echo '<tr><td><a href="' . $BASESCRIPT . '?action=plugins&operation=config&identifier=myrepeats&mod=admincp&srchuid=' . $myrepeat['uid'] . '">' . $myrepeat['user'] . '</a></td>' . '<td>' . $_DCACHE['usergroups'][$myrepeat['groupid']]['grouptitle'] . '</td>' . '<td><a href="' . $BASESCRIPT . '?action=plugins&operation=config&identifier=myrepeats&mod=admincp&srchrepeat=' . rawurlencode($myrepeat['username']) . '" title="' . htmlspecialchars($myrepeat['comment']) . '">' . $myrepeat['username'] . '</a>' . '</td>' . '<td>' . ($myrepeat['lastswitch'] ? $myrepeat['lastswitch'] : '') . '</td>' . '<td><a id="d' . $i . '" onclick="ajaxget(this.href, this.id, \'\');return false" href="' . $BASESCRIPT . '?action=plugins&operation=config&identifier=myrepeats&mod=admincp&uid=' . $myrepeat['uid'] . '&username=' . $myrepeat['usernameenc'] . '&op=lock">' . $opstr . '</a></td>' . '<td><a id="p' . $i . '" onclick="ajaxget(this.href, this.id, \'\');return false" href="' . $BASESCRIPT . '?action=plugins&operation=config&identifier=myrepeats&mod=admincp&uid=' . $myrepeat['uid'] . '&username=' . $myrepeat['usernameenc'] . '&op=delete">[' . $lang['delete'] . ']</a></td></tr>';
    }
}
showtablefooter();
