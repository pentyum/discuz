...
if (submitcheck('submit')) {
    ...
    writetocache('postawards_setting', '', getcachevars(array(
        'PACACHE' => array(
            'userright' => $userright
        )
    )));
...
showsubtitle(array(
    $palang['groupname'],
    '',
    $palang['allow_postawards'],
    $palang['allow_systemcredit'],
    $palang['allow_systemcredit_low'],
    $palang['allow_systemcredit_high'],
    $palang['allow_systemcredit_all'],
    $palang['allow_ratemode']
));

$query = $db->query("SELECT groupid, radminid, grouptitle FROM {$tablepre}usergroups WHERE radminid>0 ORDER BY groupid");

while ($group = $db->fetch_array($query)) {
    $list = showtablerow('', array(
        'class="td35"',
        'class="td35"',
        'class="td35"',
        'class="td35"',
        ...
        '<input type="checkbox" class="checkbox" name="userright[' . $group['groupid'] . '][ratemode]" id="ratemode" value="1"' . $ratemode_checked[$group['groupid']] . '>'
    ), TRUE);
    echo $list;
}
